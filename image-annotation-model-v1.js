...
function hit(item,p,tol){if(item.points){return item.points.some((b,i)=>distance(p,item.points[Math.max(0,i-1)],b)<=tol+item.width/2)}if(item.type==='arrow')return distance(p,item.a,item.b)<=tol+item.width*2;const b=bounds(item);return p.x>=b.x-tol&&p.y>=b.y-tol&&p.x<=b.x+b.w+tol&&p.y<=b.y+b.h+tol}
