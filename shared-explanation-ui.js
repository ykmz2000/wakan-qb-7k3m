...
function detailRows(c){
  ...
  if(c.correct_for_other_context)rows.push(['類題・補足',c.correct_for_other_context,'qbChoiceOtherContext']);
  if(c.examiner_distinction)rows.push(['見分けるポイント',c.examiner_distinction,'qbChoiceDistinction']);
  return rows;
}
