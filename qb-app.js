...
let sb=null,user=null,grade=null,subjects=[],subject=null,units=[],unitQuestionIndex=[],questions=[],qstate={},ratings={},screen='subjects',unitId=null,selected=new Set(),practice=[],pi=0,submitted=false,reviewOnly=false,sel=new Set();
const esc=(s='')=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const fmt=s=>{if(!s)return '';const d=new Date(s);return `${d.getFullYear()}/${d.getMonth()+1}/${d.getDate()} ${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`};
const stateOf=id=>qstate[id]||{};
function emit(){window.dispatchEvent(new CustomEvent('qb-screen-change',{detail:{screen}}))}
function setScreen(s){screen=s;render();emit();scrollTo({top:0,behavior:'smooth'})}
...
async function chooseSubject(id){
  ...
  const ids=unitQuestionIndex.map(x=>x.id);
  qstate={};ratings={};
  if(ids.length){
...
function renderUnits(){
  C.textContent=`M4 ＞ ${subject.name} ＞ 単元`;
  const all=unitQuestionIndex.length;
  const rows=[{id:'__all__',name:'すべて（収録済み）'},...units];
...
function renderPractice(){
  ...
  V.innerHTML=`<div class="card"><div class="row"><div><span class="badge">${esc(o.academic_year||'年度不明')}</span><span class="badge gray">${esc(o.exam_type||'')}</span></div><div class="meta">${pi+1}/${practice.length}</div></div><div class="qtext" style="font-size:18px;font-weight:800;margin-top:12px">${esc(q.stem)}</div>${q.instruction?`<div class="meta">${esc(q.instruction)}</div>`:''}${isText?`<div class="card" style="margin-top:12px"><div class="meta">記述・穴埋め問題</div><button id="showTextAnswer" class="secondary">解答を見る</button></div>`:`<div class="choices">${q.choices.map((c,i)=>`<button class="choice ${sel.has(i)?'sel':''}" data-c="${i}" ${submitted?'disabled':''}>${esc(c.choice_key)}. ${esc(c.choice_text)}</button>`).join('')}</div><div style="margin-top:12px"><button id="answer" class="primary" ${submitted||!sel.size?'disabled':''}>解答する</button><button id="review" class="secondary" style="margin-top:8px" ${submitted?'disabled':''}>解答せずに解説を見る</button></div>`}<div id="ans"></div><div class="nav"><button id="prev" class="btn" ${pi===0?'disabled':''}>← 前へ</button><button id="next" class="btn">${pi===practice.length-1?'終了':'次へ →'}</button></div></div>`;
  if(isText){document.getElementById('showTextAnswer').onclick=()=>{reviewOnly=true;submitted=true;drawTextAnswer(q,official)}}else{
    ...
    document.getElementById('answer').onclick=()=>submitAnswer(q);
...
function move(d){pi+=d;submitted=false;reviewOnly=false;sel=new Set();renderPractice();scrollTo({top:0,behavior:'smooth'})}
function finish(){practice=[];pi=0;submitted=false;reviewOnly=false;sel=new Set();setScreen('problems')}
...
M.querySelectorAll('[data-mode]').forEach(b=>b.onclick=()=>{const ids=[...selected];practice=b.dataset.mode==='shuffle'?ids.sort(()=>Math.random()-.5):ids;pi=0;submitted=false;reviewOnly=false;sel=new Set();M.classList.add('hidden');setScreen('practice')});document.getElementById('cancel').onclick=()=>M.classList.add('hidden');
...
window.qbGetScreen=()=>screen;window.qbOpenSubjects=()=>setScreen('subjects');window.showGradeScreen=()=>setScreen('grades');
