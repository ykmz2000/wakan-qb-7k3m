...
function renderProblems(){
  ...
  V.innerHTML=`<div class="card"><div class="title">${esc(u?.name||'')}</div><div class="meta">${questions.length}問</div></div><div class="card"><div class="row"><b>問題一覧</b><button id="toggleAll" class="btn">${selected.size===questions.length?'すべて解除':'すべて選択'}</button></div>${questions.map((q,i)=>{const st=stateOf(q.id),o=q.occ[0]||{};const status=st.has_answered?(st.last_is_correct?'正解':'不正解'):st.has_viewed_explanation?'解説のみ':'未閲覧';return `<div class="problem"><div class="qid">${i+1}</div><div><div class="qtext">${esc(q.stem)}</div><div class="meta">${q.occ.map(x=>x.academic_year).filter(Boolean).join('・')}｜${esc(o.exam_type||'')}｜${esc(o.original_question_number||'')}｜${status}</div></div><div class="pick"><input type="checkbox" data-q="${q.id}" ${selected.has(String(q.id))?'checked':''}></div></div>`}).join('')}</div><div class="sticky"><button id="start" class="primary">演習開始（${selected.size}問）</button></div>`;
...
async function resumeSession(s){
  document.getElementById('qbResumeV2')?.remove();const target=subjects.find(x=>x.id===s.subject_id);if(!target)throw new Error('保存された科目が見つかりません');
  const scope=s.metadata?.unit_scope||(s.unit_id||'__all__');
  if(subject?.id!==target.id)await chooseSubject(target.id);
  const requested=(s.question_ids||[]).map(String),availableHere=new Set(questions.map(q=>String(q.id))),canReuse=unitId===scope&&requested.every(id=>availableHere.has(id));
  if(!canReuse)await loadQuestionsForUnit(scope);
  const available=new Set(questions.map(q=>String(q.id)));practice=requested.filter(id=>available.has(id));if(!practice.length)throw new Error('保存された問題を読み込めません');
  selected=new Set(practice);pi=Math.max(0,Math.min(Number(s.current_index)||0,practice.length-1));practiceMode=s.mode||'ordered';sessionId=s.id;submitted=false;reviewOnly=false;sel=new Set();setScreen('practice')
}
M.querySelectorAll('[data-mode]').forEach(b=>b.onclick=async()=>{
  const mode=b.dataset.mode||'ordered',available=questions.map(q=>String(q.id)),chosen=available.filter(id=>selected.has(id));if(!chosen.length)return;
  practiceMode=mode;M.querySelectorAll('[data-mode]').forEach(x=>x.disabled=true);
  try{practice=typeof window.qbPreparePracticeIds==='function'?await window.qbPreparePracticeIds(chosen,subject?.id,available,mode):(mode==='shuffle'?[...chosen].sort(()=>Math.random()-.5):chosen)}catch(e){console.error('series-aware practice prepare',e);practice=mode==='shuffle'?[...chosen].sort(()=>Math.random()-.5):chosen}
  pi=0;submitted=false;reviewOnly=false;sel=new Set();sessionId=null;M.classList.add('hidden');M.querySelectorAll('[data-mode]').forEach(x=>x.disabled=false);setScreen('practice');saveSession(true)
});document.getElementById('cancel').onclick=()=>M.classList.add('hidden');
