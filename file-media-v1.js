...
async function validate(file){if(!file?.size||file.size>MAX)throw Error('ファイルは100MiB以下にしてください。');if(!supported(file))throw Error('画像またはPDFを選択してください。');if(isPDF(file)){const header=new TextDecoder().decode(await file.slice(0,1024).arrayBuffer());if(!header.includes('%PDF-'))throw Error('PDFの内容を確認できません。');return new File([file],file.name||'資料.pdf',{type:'application/pdf'})}const type=/^image\//.test(file.type||'')?file.type:imageMime(file.name);return type&&type!==file.type?new File([file],file.name||'画像',{type}):file}
let enginePromise;
...
function alternateStorageSource(source){if(typeof source!=='string')return '';const match=source.match(/\/storage\/v1\/object\/(public|sign)\/(question-media|question-images)\//);if(!match)return '';const bucket=match[2]==='question-media'?'question-images':'question-media';return source.replace(match[0],`/storage/v1/object/${match[1]}/${bucket}/`)}
async function documentTask(source){const pdf=await engine(),base=new URL('./vendor/pdfjs/',document.baseURI).href,alternate=alternateStorageSource(source);let activeSource=source,destroyed=false;const create=async value=>{const options={isEvalSupported:false,disableAutoFetch:true,disableStream:true,cMapUrl:base+'cmaps/',cMapPacked:true,standardFontDataUrl:base+'standard_fonts/',wasmUrl:base+'wasm/'};if(value instanceof Blob){if(value.size>MAX)throw Error('PDFは100MiB以下にしてください。');options.data=new Uint8Array(await value.arrayBuffer())}else options.url=value;return pdf.getDocument(options)};let active=await create(source);const primary=active,promise=primary.promise.catch(async error=>{if(!alternate||destroyed)throw error;await primary.destroy().catch(()=>{});if(destroyed)throw error;activeSource=alternate;active=await create(alternate);return active.promise});return{promise,get source(){return activeSource},destroy:async()=>{destroyed=true;return active?.destroy()}}}
async function previewPage(source,{maxPixels=8000000,maxEdge=8192,maxScale=2}={}){let task,canvas;try{task=await documentTask(source);const doc=await task.promise,page=await doc.getPage(1),raw=page.getViewport({scale:1}),vp=page.getViewport({scale:Math.min(maxScale,Math.sqrt(maxPixels/(raw.width*raw.height)),maxEdge/Math.max(raw.width,raw.height))});canvas=document.createElement('canvas');canvas.width=Math.ceil(vp.width);canvas.height=Math.ceil(vp.height);await page.render({canvasContext:canvas.getContext('2d'),viewport:vp}).promise;const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/png'));if(!blob)throw Error('PDFのプレビューを生成できません。');return {blob,pages:doc.numPages}}finally{if(canvas)canvas.width=canvas.height=1;await task?.destroy().catch(()=>{})}}
...
function enqueueThumbnail(node,force=false){
 ...
 queuedNodes.add(node);if(st)st.queued=true;if(nearViewport(node))thumbnailQueue.unshift(node);else thumbnailQueue.push(node);void drain()
}
const visible=new IntersectionObserver(entries=>{for(const e of entries)if(e.isIntersecting)enqueueThumbnail(e.target)},{rootMargin:'120px'});
...
async function cardDocument(card){
 ...
 state={task:null,promise:null};state.promise=(async()=>{state.task=await documentTask(card.dataset.pdfSrc);const doc=await state.task.promise;if(state.task.source&&state.task.source!==card.dataset.pdfSrc)card.dataset.pdfSrc=state.task.source;return doc})().catch(async e=>{cardDocuments.delete(card);await state.task?.destroy().catch(()=>{});throw e});cardDocuments.set(card,state);return state.promise
}
...
async function drainWorker(){
 ...
 try{
  ...
  if(!st&&card.dataset.pdfPassive!=='true'&&!card.closest('.qbLibraryImageButton,.qbripItem')){
   ...
   let firstSlot=null;for(let n=1;n<=doc.numPages;n++){const page=await doc.getPage(n),vp=page.getViewport({scale:1}),slot=element('span','qbPdfInlinePage');if(!firstSlot)firstSlot=slot;slot.dataset.inlinePage=n;slot.setAttribute('role','button');slot.tabIndex=0;slot.setAttribute('aria-label',`${n}ページを拡大表示`);slot.style.aspectRatio=vp.width+'/'+vp.height;slot.style.setProperty('--qb-page-width',Math.min(380,400*vp.width/vp.height)+'px');poster.append(slot);inlineStates.set(slot,{card,visible:false,width:0,token:0,queued:false,rendered:false});inlineObserver.observe(slot);inlineResize.observe(slot)}
   visible.unobserve(card);retryCounts.delete(card);card.querySelector('.qbPdfCaption').textContent=`PDF · ${doc.numPages}ページ · タップして拡大`;if(firstSlot){inlineStates.get(firstSlot).visible=true;enqueueThumbnail(firstSlot)}continue;
  }
  const measuredWidth=st?node.clientWidth:Math.max(64,Math.min(360,card.clientWidth||card.parentElement?.clientWidth||160));
  // During a question transition WebKit can report zero width for a newly inserted
  // page. Never freeze that tiny canvas and stretch it into a blurred thumbnail.
  if(st&&measuredWidth<32){st.rendered=false;const attempt=(st.measureRetries||0)+1;st.measureRetries=attempt;if(attempt<=4)setTimeout(()=>enqueueThumbnail(node,true),attempt*60);continue}
  ...
  const coverHeight=card.closest('.qbripItem')?120:480,cssScale=st?measuredWidth/raw.width:Math.min(measuredWidth/raw.width,coverHeight/raw.height),density=Math.min(devicePixelRatio||1,3),scale=Math.min(cssScale*density,Math.sqrt(8000000/(raw.width*raw.height)),8192/Math.max(raw.width,raw.height));
...
async function open(source,{pickPage=false,mediaOrigin=null,initialPage=1}={}){
 ...
 const MAX_PDF_ZOOM=12,MAX_PDF_RENDER_PIXELS=24000000,MAX_PDF_RENDER_EDGE=12288;
 let gestureTimer;const pointers=new Map();let gesture=null,swipe=null,multiTouch=false;
 let task,doc,renderTask,closed=false,busy=false,pageNumber=1,zoom=1,sequence=0,thumbnailRun=0,thumbnailJob=null,currentCanvas=null,resolveResult,resizeObserver,resizeTimer;
 const result=new Promise(resolve=>resolveResult=resolve);
 async function close(value=null){if(closed||busy)return;closed=true;clearTimeout(gestureTimer);pointers.clear();clearTimeout(resizeTimer);resizeObserver?.disconnect();sequence++;thumbnailRun++;renderTask?.cancel();for(const canvas of modal.querySelectorAll('canvas'))canvas.width=canvas.height=1;stage.replaceChildren();nav.replaceChildren();currentCanvas=null;await task?.destroy().catch(()=>{});modal.remove();background.forEach(([n,v])=>{if(n.isConnected)n.inert=v});document.documentElement.style.overflow=overflow;active=null;if(origin?.isConnected)origin.focus({preventScroll:true});resolveResult(value)}
 ...
 async function openEditor(order=null){
  if(!runEditor)return;const originalPage=pageNumber,target=order?order.indexOf(originalPage-1)+1:originalPage;await close();try{const saved=await runEditor(originalPage,order);if(saved instanceof Blob){await new Promise(r=>setTimeout(r,100));const next=editorRow?[...document.querySelectorAll('[data-row],[data-id]')].find(w=>(w.dataset.row||w.dataset.id)===editorRow):document.querySelector('.qbLibraryDetailImage')?.qbPdfReplacement;await open(saved,{mediaOrigin:next||editorHost,initialPage:target})}else if(order)await open(source,{mediaOrigin:editorHost,initialPage:originalPage})}catch(e){alert('PDF編集を開けませんでした：'+e.message)}
 }
 function enablePageReorder(strip){
  if(!runEditor||doc.numPages<2)return;strip.dataset.reorderable='true';strip.setAttribute('aria-label','ページ一覧。番号をドラッグして並べ替え');let drag=null,suppress=false;
  const clear=()=>{strip.querySelectorAll('[data-dragging],[data-drop]').forEach(node=>{delete node.dataset.dragging;delete node.dataset.drop});drag=null};
  strip.addEventListener('pointerdown',e=>{const b=e.target.closest('[data-page]');if(!b||e.button!==0)return;drag={button:b,id:e.pointerId,x:e.clientX,y:e.clientY,active:false,changed:false};try{b.setPointerCapture(e.pointerId)}catch{}});
  strip.addEventListener('pointermove',e=>{if(!drag||drag.id!==e.pointerId)return;const dx=e.clientX-drag.x,dy=e.clientY-drag.y;if(!drag.active){if(Math.hypot(dx,dy)<8)return;if(Math.abs(dy)>Math.abs(dx)){clear();return}drag.active=true;drag.button.dataset.dragging='true'}e.preventDefault();const hit=document.elementFromPoint(e.clientX,e.clientY)?.closest('[data-page]');strip.querySelectorAll('[data-drop]').forEach(node=>delete node.dataset.drop);if(!hit||hit===drag.button||!strip.contains(hit))return;hit.dataset.drop='true';const buttons=[...strip.querySelectorAll('[data-page]')],from=buttons.indexOf(drag.button),to=buttons.indexOf(hit);if(from<to)strip.insertBefore(drag.button,hit.nextSibling);else strip.insertBefore(drag.button,hit);drag.changed=true;const r=strip.getBoundingClientRect();if(e.clientX<r.left+42)strip.scrollLeft-=18;else if(e.clientX>r.right-42)strip.scrollLeft+=18});
  const end=e=>{if(!drag||drag.id!==e.pointerId)return;const changed=drag.active&&drag.changed,order=[...strip.querySelectorAll('[data-page]')].map(b=>Number(b.dataset.page)-1);suppress=drag.active;clear();if(changed)void openEditor(order)};
  strip.addEventListener('pointerup',end);strip.addEventListener('pointercancel',clear);strip.addEventListener('lostpointercapture',e=>{if(drag&&drag.id===e.pointerId&&!drag.active)clear()});strip.addEventListener('click',e=>{if(!suppress)return;suppress=false;e.preventDefault();e.stopImmediatePropagation()},true);
 }

 async function show(number=pageNumber,preserve=false){if(!doc||closed)return;pageNumber=Math.max(1,Math.min(doc.numPages,number));const seq=++sequence;renderTask?.cancel();const old=renderTask;try{await old?.promise}catch{}if(closed||seq!==sequence)return;status.textContent='読み込み中…';try{const page=await doc.getPage(pageNumber);if(closed||seq!==sequence)return;const raw=page.getViewport({scale:1}),cssScale=Math.min(Math.max(1,stage.clientWidth-48)/raw.width,Math.max(1,stage.clientHeight-48)/raw.height)*zoom;const pixelScale=Math.min(cssScale*Math.min(devicePixelRatio||1,2),Math.sqrt(MAX_PDF_RENDER_PIXELS/(raw.width*raw.height)),MAX_PDF_RENDER_EDGE/Math.max(raw.width,raw.height));const viewport=page.getViewport({scale:pixelScale}),canvas=element('canvas','qbPdfPage');canvas.width=Math.ceil(viewport.width);canvas.height=Math.ceil(viewport.height);canvas.style.width=raw.width*cssScale+'px';canvas.style.height=raw.height*cssScale+'px';const job=page.render({canvasContext:canvas.getContext('2d'),viewport});renderTask=job;await job.promise;if(closed||seq!==sequence){canvas.width=canvas.height=1;return}const scroll={left:stage.scrollLeft,top:stage.scrollTop};if(currentCanvas)currentCanvas.width=currentCanvas.height=1;currentCanvas=canvas;const frame=element('div','qbPdfPageFrame');frame.append(canvas);stage.replaceChildren(frame);status.textContent=`${pageNumber} / ${doc.numPages}ページ`;const gallery=mediaOrigin&&window.QBMediaGallery?.items(mediaOrigin);previous.disabled=pageNumber===1&&(pickPage||!gallery||gallery.index===0);next.disabled=pageNumber===doc.numPages&&(pickPage||!gallery||gallery.index===gallery.images.length-1);const selected=nav.querySelector(`[data-page="${pageNumber}"]`);nav.querySelectorAll('[data-page]').forEach(b=>b.setAttribute('aria-current',String(b===selected)));selected?.scrollIntoView({block:'nearest',inline:'nearest'});if(preserve){stage.scrollLeft=scroll.left;stage.scrollTop=scroll.top}else stage.scrollTop=stage.scrollLeft=0;void pageThumbs()}catch(e){if(!closed&&seq===sequence&&e.name!=='RenderingCancelledException')status.textContent='ページを表示できませんでした。別のページを選ぶか再試行してください。'}}
 async function pageThumbs(){
   if(thumbnailJob?.run===thumbnailRun)return thumbnailJob.promise;
   const run=thumbnailRun,current=pageNumber,buttons=[...nav.querySelectorAll('[data-page]')].sort((a,b)=>Math.abs(Number(a.dataset.page)-current)-Math.abs(Number(b.dataset.page)-current)||Number(a.dataset.page)-Number(b.dataset.page));
   const promise=(async()=>{for(const b of buttons){if(closed||run!==thumbnailRun||!b.isConnected)return;if(b.querySelector('canvas'))continue;const n=Number(b.dataset.page);try{const page=await doc.getPage(n);if(closed||run!==thumbnailRun||!b.isConnected)return;const raw=page.getViewport({scale:1}),scale=Math.min(80/raw.width,110/raw.height),vp=page.getViewport({scale:scale*Math.min(window.devicePixelRatio||1,3)}),c=document.createElement('canvas');c.width=Math.ceil(vp.width);c.height=Math.ceil(vp.height);c.style.width=raw.width*scale+'px';c.style.height=raw.height*scale+'px';await page.render({canvasContext:c.getContext('2d'),viewport:vp}).promise;if(closed||run!==thumbnailRun||!b.isConnected){c.width=c.height=1;return}b.prepend(c)}catch{if(closed||run!==thumbnailRun)return}}})();
   thumbnailJob={run,promise};try{await promise}finally{if(thumbnailJob?.promise===promise)thumbnailJob=null}
 }
 ...
 stage.addEventListener('pointermove',e=>{
  ...
  if(pts.length>1&&gesture.distance){
   const center={x:(pts[0].x+pts[1].x)/2,y:(pts[0].y+pts[1].y)/2};zoom=Math.max(.5,Math.min(MAX_PDF_ZOOM,gesture.zoom*Math.hypot(pts[1].x-pts[0].x,pts[1].y-pts[0].y)/gesture.distance));
 ...
 const previous=button('前のページ',()=>go(-1)),next=button('次のページ',()=>go(1));head.append(previous,next,button('全体を表示',()=>{zoom=1;void show()}),button('−',()=>{zoom=Math.max(.5,zoom/1.25);void show()}),button('＋',()=>{zoom=Math.min(MAX_PDF_ZOOM,zoom*1.25);void show()}));
 ...
 modal.addEventListener('keydown',e=>{e.stopPropagation();if(e.key==='Escape'){e.preventDefault();if(questionPreview?.closeIfOpen())return;void close()}if(e.key==='ArrowLeft'||e.key==='ArrowRight'){e.preventDefault();void go(e.key==='ArrowRight'?1:-1)}if(e.key==='Tab'){const nodes=[...modal.querySelectorAll('button,a')].filter(n=>!n.disabled&&!n.hidden&&n.getClientRects().length),first=nodes[0],last=nodes.at(-1);if(e.shiftKey&&(document.activeElement===first||document.activeElement===panel)){e.preventDefault();last?.focus()}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first?.focus()}}});modal.addEventListener('click',e=>{if(e.target===modal)void close();else e.stopPropagation()});panel.focus();
 const retry=button('再試行',()=>loadDocument());retry.hidden=true;head.append(retry);
 async function loadDocument(){
   retry.hidden=true;status.textContent='PDFを読み込み中…';thumbnailRun++;doc=null;
   await task?.destroy().catch(()=>{});if(closed)return;
   try{task=await documentTask(source);if(closed){await task.destroy();return}doc=await task.promise;if(closed)return;if(task.source&&task.source!==source){source=task.source;if(mediaOrigin?.dataset)mediaOrigin.dataset.pdfSrc=source}status.classList.remove('qbPdfStatusError');nav.replaceChildren();const strip=element('div','qbPdfPageStrip');strip.setAttribute('aria-label','全ページのプレビュー');nav.append(strip);for(let n=1;n<=doc.numPages;n++){const b=button(String(n),()=>show(n));b.dataset.page=n;b.setAttribute('aria-label',`${n}ページを表示`);strip.append(b)}enablePageReorder(strip);await show(initialPage);void pageThumbs()}catch(e){if(!closed){doc=null;status.classList.add('qbPdfStatusError');status.textContent='PDFを開けませんでした。暗号化・破損・通信状況を確認してください。';retry.hidden=false}}
 }
