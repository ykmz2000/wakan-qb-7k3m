...
async function drainWorker(){
 ...
 try{
  ...
  const measuredWidth=st?node.clientWidth:0;
...
async function open(source,{pickPage=false,mediaOrigin=null,initialPage=1}={}){
 ...
 const MAX_PDF_ZOOM=8,MAX_PDF_RENDER_PIXELS=24000000,MAX_PDF_RENDER_EDGE=12288;
 ...
 async function close(value=null){if(closed||busy)return;closed=true;clearTimeout(gestureTimer);pointers.clear();clearTimeout(resizeTimer);resizeObserver?.disconnect();sequence++;thumbnailRun++;renderTask?.cancel();await task?.destroy().catch(()=>{});modal.remove();background.forEach(([n,v])=>{if(n.isConnected)n.inert=v});document.documentElement.style.overflow=overflow;active=null;if(origin?.isConnected)origin.focus({preventScroll:true});resolveResult(value)}
