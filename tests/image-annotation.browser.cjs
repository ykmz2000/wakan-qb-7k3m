...
async function integration(browser,name){
  ...
  await p.locator('.qbPublicImageWrap .qbImageWriteActions').first().waitFor({state:'attached'});assert.equal(await p.locator('.qbPublicImageWrap .qbImageWriteActions').first().isVisible(),true);await p.locator('[data-ade-v2="overview"]').click();await p.locator('.qbInlineRich').waitFor();assert.ok(await p.locator('.qbPublicImageWrap').first().getByRole('button',{name:'トリミング',exact:true}).isVisible());assert.ok(await p.locator('.qbPublicImageWrap').first().getByRole('button',{name:'書き込み',exact:true}).isVisible());assert.equal(await p.locator('.qbDrawModal').count(),0);await p.locator('.qbInlineCancel').click();assert.equal(await p.locator('.qbPublicImageWrap .qbImageWriteActions').first().isVisible(),true);
