...
async function integration(browser,name){
  ...
  await p.locator('.qbPublicImageWrap .qbImageWriteActions').first().waitFor({state:'attached'});assert.equal(await p.locator('.qbPublicImageWrap .qbImageWriteActions').first().isVisible(),false);await p.locator('[data-ade-v2="overview"]').click();await p.locator('.qbInlineRich').waitFor();assert.ok(await p.locator('.qbPublicImageWrap').first().getByRole('button',{name:'トリミング',exact:true}).isVisible());assert.ok(await p.locator('.qbPublicImageWrap').first().getByRole('button',{name:'書き込み',exact:true}).isVisible());assert.equal(await p.locator('.qbDrawModal').count(),0);await p.locator('.qbInlineCancel').click();assert.equal(await p.locator('.qbPublicImageWrap .qbImageWriteActions').first().isVisible(),false);
  ...
  });assert.equal(await u.evaluate(()=>highResolutionWidth),2400);assert.ok(exportResult.height>4000);assert.equal(exportResult.type,'image/png');assert.ok(exportResult.lines.includes('問題末尾')&&exportResult.lines.includes('b. 選択肢末尾')&&exportResult.lines.includes('a'));assert.ok(exportResult.unchanged);assert.equal(exportResult.nonChoice,'空欄1：模範解答\n補足：補足情報');
