...
async function pixels(p,points){return p.evaluate(async points=>{
  const image=await new Promise((resolve,reject)=>{const i=new Image(),u=URL.createObjectURL(drawResult);i.onload=()=>{URL.revokeObjectURL(u);resolve(i)};i.onerror=reject;i.src=u});const c=document.createElement('canvas');c.width=image.width;c.height=image.height;const x=c.getContext('2d');x.drawImage(image,0,0);return{width:c.width,height:c.height,type:drawResult.type,colors:points.map(([a,b])=>[...x.getImageData(a,b,1,1).data])};
},points)}
async function core(browser,name){
  ...
  await p.route('**/*',r=>r.request().url()==='https://qb-draw.test/'?r.fulfill({contentType:'text/html',body:'<!doctype html><style>*{box-sizing:border-box}body{margin:0}</style><button>前の画面</button>'}):r.abort());await p.goto('https://qb-draw.test/');await install(p);
  ...
  await p.evaluate(({a,b,palm})=>{const c=document.querySelector('.qbDrawCanvas'),event=(type,id,kind,point)=>c.dispatchEvent(new PointerEvent(type,{bubbles:true,cancelable:true,pointerId:id,pointerType:kind,button:0,buttons:type==='pointerup'?0:1,clientX:point.x,clientY:point.y}));event('pointerdown',41,'pen',a);event('pointerdown',42,'touch',palm);event('pointermove',41,'pen',b);event('pointerup',42,'touch',palm);event('pointerup',41,'pen',b)}, {a,b,palm});
...
async function integration(browser,name){
  ...
  const exportResult=await u.evaluate(async()=>{
    ...
    const image=await new Promise((resolve,reject)=>{const i=new Image(),url=URL.createObjectURL(result.blob);i.onload=()=>{URL.revokeObjectURL(url);resolve(i)};i.onerror=reject;i.src=url});
