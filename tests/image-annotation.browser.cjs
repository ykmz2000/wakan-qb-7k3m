...
async function integration(browser,name){
  ...
  assert.equal(await publicImage.getByRole('button',{name:'元画像に戻す',exact:true}).count(),0);
  await publicImage.getByRole('button',{name:'画像編集',exact:true}).click();await p.locator('.qbDrawSave:enabled').waitFor();p.once('dialog',d=>d.dismiss());await p.locator('.qbDrawReset').click();assert.equal(await p.evaluate(id=>testDB.question_images.find(r=>r.id===id).image_path,deleteId),changedPath);
  p.once('dialog',d=>d.accept());await p.locator('.qbDrawReset').click();await p.locator('.qbDrawSave:enabled').waitFor();await p.locator('.qbDrawSave').click();await p.locator('.qbDrawModal').waitFor({state:'detached'});await p.waitForFunction(({id,changedPath})=>{const r=testDB.question_images.find(r=>r.id===id);return r.image_path!==changedPath&&!!r.original_image_path},{id:deleteId,changedPath});
