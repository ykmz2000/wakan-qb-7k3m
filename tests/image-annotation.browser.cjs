...
async function core(browser,name){
  ...
  await p.getByRole('button',{name:'トリミング',exact:true}).click();await p.waitForFunction(()=>document.querySelector('.qbCropSave')?.disabled===false);
...
async function integration(browser,name){
  ...
  await p.locator('.qbPublicImageWrap .qbImageWriteActions').first().waitFor({state:'attached'});assert.equal(await p.locator('.qbPublicImageWrap .qbImageWriteActions').first().isVisible(),true);await p.locator('[data-ade-v2="overview"]').click();await p.locator('.qbInlineRich').waitFor();assert.ok(await p.locator('.qbPublicImageWrap').first().getByRole('button',{name:'トリミング',exact:true}).isVisible());assert.ok(await p.locator('.qbPublicImageWrap').first().getByRole('button',{name:'画像編集',exact:true}).isVisible());assert.equal(await p.locator('.qbDrawModal').count(),0);await p.locator('.qbInlineCancel').click();assert.equal(await p.locator('.qbPublicImageWrap .qbImageWriteActions').first().isVisible(),true);
  const readingImage=p.locator('.qbPublicImageWrap').first();await readingImage.getByRole('button',{name:'トリミング',exact:true}).click();await p.locator('.qbCropCancel').click();await p.locator('.qbCropModal').waitFor({state:'detached'});assert.equal(await p.locator('.qbInlineRich').count(),0);
