...
async function run(browser,label,url){
 ...
 await photoMenu.getByRole('button',{name:'複製',exact:true}).click();await p.waitForFunction(()=>document.querySelector('.qbDrawStatus')?.textContent==='写真を編集可能なまま複製しました。');
 // The exact duplicate is the topmost object; deleting it must leave the original intact.
 await p.evaluate(()=>{const c=document.querySelector('.qbDrawCanvas'),r=document.querySelector('.qbDrawStage').getBoundingClientRect(),x=r.x+r.width/2,y=r.y+r.height/2;c.dispatchEvent(new PointerEvent('pointerdown',{pointerId:83,pointerType:'touch',clientX:x,clientY:y,bubbles:true,button:0}))});await photoMenu.waitFor();await p.evaluate(()=>{const c=document.querySelector('.qbDrawCanvas'),r=document.querySelector('.qbDrawStage').getBoundingClientRect();c.dispatchEvent(new PointerEvent('pointerup',{pointerId:83,pointerType:'touch',clientX:r.x+r.width/2,clientY:r.y+r.height/2,bubbles:true,button:0}))});await photoMenu.getByRole('button',{name:'削除',exact:true}).click();
 // Empty canvas: one-finger hold and stationary two-finger tap open paste/add actions.
 const emptyMenu=p.getByRole('dialog',{name:'貼り付け・追加',exact:true});
 await p.evaluate(()=>{const c=document.querySelector('.qbDrawCanvas'),r=document.querySelector('.qbDrawStage').getBoundingClientRect();c.dispatchEvent(new PointerEvent('pointerdown',{pointerId:84,pointerType:'touch',clientX:r.x+18,clientY:r.y+18,bubbles:true,button:0}))});await emptyMenu.waitFor();await p.evaluate(()=>{const c=document.querySelector('.qbDrawCanvas'),r=document.querySelector('.qbDrawStage').getBoundingClientRect();c.dispatchEvent(new PointerEvent('pointerup',{pointerId:84,pointerType:'touch',clientX:r.x+18,clientY:r.y+18,bubbles:true,button:0}))});await emptyMenu.getByRole('button',{name:'閉じる',exact:true}).click();
 await p.evaluate(()=>{const c=document.querySelector('.qbDrawCanvas'),r=document.querySelector('.qbDrawStage').getBoundingClientRect(),fire=(type,id,dx)=>c.dispatchEvent(new PointerEvent(type,{pointerId:id,pointerType:'touch',clientX:r.x+18+dx,clientY:r.y+18,bubbles:true,button:0}));fire('pointerdown',85,-3);fire('pointerdown',86,3);fire('pointerup',85,-3);fire('pointerup',86,3)});await emptyMenu.waitFor();await emptyMenu.getByRole('button',{name:'ペースト',exact:true}).click();await p.waitForFunction(()=>document.querySelector('.qbDrawStatus')?.textContent==='画像を追加しました');
 ...
 // A drag order supplied by the enlarged viewer opens as an unsaved edit and
 // keeps the originally viewed page selected at its new position.
 await p.evaluate(()=>{window.saved=null;QBPDFEditor.open(original,{initialPage:1,initialOrder:[1,0,2],onSave:async b=>window.saved=b})});await p.waitForFunction(()=>document.querySelector('.qbPdfStatus')?.textContent==='2 / 3ページ · 未保存');
 await p.getByRole('button',{name:'PDFを保存',exact:true}).click();await p.waitForFunction(()=>saved instanceof Blob);await p.locator('.qbPdfModal').waitFor({state:'detached'});
 const reordered=await p.evaluate(async()=>{const t=await QBFiles.documentTask(saved),d=await t.promise,colors=[];for(let n=1;n<=2;n++){const page=await d.getPage(n),v=page.getViewport({scale:.2}),c=document.createElement('canvas');c.width=v.width;c.height=v.height;await page.render({canvasContext:c.getContext('2d'),viewport:v}).promise;colors.push([...c.getContext('2d').getImageData(1,1,1,1).data].slice(0,3))}await t.destroy();return colors});assert.deepEqual(reordered,[[0,255,0],[255,0,0]]);
 // iPhone fallback: when direct clipboard reading is unavailable, the paste
 // target is visible and accepts an image paste to create a one-page PDF.
 await p.evaluate(()=>{Object.defineProperty(navigator,'clipboard',{configurable:true,value:{}});window.created=null;QBPDFEditor.create({onSave:async b=>window.created=b})});await p.getByRole('button',{name:'ペーストから作成',exact:true}).click();
