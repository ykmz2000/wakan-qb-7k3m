...
async function run(browser,name){
  ...
  await p.locator('.qbDrawModal').getByRole('button',{name:'キャンセル',exact:true}).click();assert.deepEqual(errors,[]);await p.close();
