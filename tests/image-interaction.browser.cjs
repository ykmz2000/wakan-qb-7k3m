...
async function run(browser,name){
  ...
  await p.getByLabel('線の太さ',{exact:true}).selectOption('5');const map2=await mapping(p);await drag(p,map2,[[100,100],[300,250]]);await p.locator('.qbDrawSave').click();await p.locator('.qbDrawModal').waitFor({state:'detached'});// Count the 10px stroke across a scanline instead of sampling an antialiased edge.
  const thick=await pixels(p,Array.from({length:21},(_,i)=>[150,90+i]));const blueRows=thick.colors.map((c,i)=>c[2]>150&&c[0]<150?i:-1).filter(i=>i>=0);assert.ok(blueRows.length>=9&&blueRows.length<=11,`expected a 10px stroke: ${JSON.stringify(thick.colors)}`);assert.equal(blueRows.at(-1)-blueRows[0]+1,blueRows.length);assert.deepEqual(thick.colors[0],[255,255,255,255]);assert.deepEqual(thick.colors.at(-1),[255,255,255,255]);
