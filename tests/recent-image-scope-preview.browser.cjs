...
async function run(browser,name){
  ...
  await p.waitForFunction(()=>{const imageHosts=[...document.querySelectorAll('.qbripItemHost')].filter(host=>host.querySelector('.qbripItem>img'));return imageHosts.length===imageHosts.filter(host=>host.querySelector(':scope>.qbMediaShareButton')).length});assert.ok(await p.locator('.qbripItemHost>.qbMediaShareButton').count()>1);pass('each loaded image keeps its share action while recent PDF placeholders avoid eager rendering');
