...
async function run(browser,label){
 ...
 await p.getByLabel('ファイル名',{exact:true}).fill('戻るだけで保存');await p.getByLabel('キーワード',{exact:true}).fill('lung cancer');
 ...
 await p.locator('.qbLibraryImageButton').first().click();await edit.click();await p.getByLabel('ファイル名',{exact:true}).fill('失敗しても保持');
 ...
 await back.click();await p.getByText('保存に失敗しました',{exact:true}).waitFor();assert.equal(await p.getByLabel('ファイル名',{exact:true}).inputValue(),'失敗しても保持');assert.equal(await back.isDisabled(),false);
 ...
 const review=p.getByRole('dialog',{name:'追加したファイルを確認',exact:true});await review.waitFor();await review.getByLabel('ファイル名',{exact:true}).fill('追加直後も戻るだけで保存');await review.getByRole('button',{name:'一覧に戻る',exact:true}).click();await review.waitFor({state:'detached'});
