...
async function run(browser,label){
  ...
  await page.evaluate(()=>{localStorage.setItem('qb-image-editor-settings-v1','{}');window.pq=()=>({id:'q-test'});const c=document.createElement('canvas');c.width=800;c.height=600;c.getContext('2d').fillRect(0,0,1,1);window.QBQuestionExport={render:async()=>({blob:await new Promise(r=>c.toBlob(r,'image/png'))})};QBImageEditor.open(c.toDataURL())});
