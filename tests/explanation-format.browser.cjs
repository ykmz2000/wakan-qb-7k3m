...
const scripts=['shared-explanation-ui.js','qb-media-notes-v2.js','explanation-format-v1.js','admin-direct-edit-v2.js','qb-editor-layout-polish.js','image-viewer.js','official-edit-images.js','official-answer-inline-edit-v2.js','fill-blank-v2.js','theme-system-v1.js','theme-coverage-v2.js'];
...
async function setup(browser,{role='admin',mode='single',db=null}={}){
  ...
  await page.evaluate(({role,mode,seed})=>{
    ...
    const q={id:'q1',stem:'検証用の問題 [A]',answer_mode:mode,explanation_overview:'本文のキーワードと補足 < 5。\n2行目',examiner_intent:'この設問の区別点',exam_summary:'覚えておく総括',medical_verification_note:'資料の確認事項',explanation_formatting:null,updated_at:stamp,occ:[{id:'o1',question_id:'q1',official_answer:{A:'模範解答'},official_answer_formatting:null,updated_at:stamp}]};
...
async function run(browser,name){
  ...
  const editLayout=await page.locator('[data-ade-v2="overview"]').evaluate(button=>{const heading=button.parentElement,card=heading.parentElement,body=card.querySelector(':scope > .line'),cardStyle=getComputedStyle(card),bodyBox=body.getBoundingClientRect(),contentRight=card.getBoundingClientRect().right-parseFloat(cardStyle.paddingRight);return{inHeading:heading.tagName==='B'&&heading.classList.contains('adeEditHeading'),noDirectButton:!card.querySelector(':scope > .adeEditBtnV2'),fullBodyWidth:Math.abs(bodyBox.right-contentRight)<1.1,paddingRight:cardStyle.paddingRight}});
  assert.deepEqual(editLayout,{inHeading:true,noDirectButton:true,fullBodyWidth:true,paddingRight:'15px'});pass('section edit controls stay inside bold headings without narrowing body text');
  ...
  await page.evaluate(text=>{testDB.questions[0].explanation_overview=text;testDB.questions[0].explanation_formatting=null;testQ.explanation_overview=text;testQ.explanation_formatting=null;window.dispatchEvent(new CustomEvent('qb-content-updated',{detail:{questionId:'q1',type:'text',field:'explanation_overview'}}))},original);await page.waitForTimeout(100);
  ...
  const fill=await setup(browser,{mode:'fill_blank'});assert.equal(await fill.page.locator('#ans > .card > b').filter({hasText:'問題文のポイント'}).count(),1);assert.equal(await fill.page.locator('#ans > .card > b').filter({hasText:'■ 解説'}).count(),0);await fill.page.locator('.oaiEditBtn').waitFor();
  const answerEditLayout=await fill.page.locator('.oaiEditBtn').evaluate(button=>{const heading=button.parentElement,group=heading.parentElement,line=group.querySelector(':scope > .fbAnswerLine'),groupBox=group.getBoundingClientRect(),lineBox=line.getBoundingClientRect();return{inHeading:heading.tagName==='B'&&heading.classList.contains('oaiEditHeading'),noDirectButton:!group.querySelector(':scope > .oaiEditBtn'),paddingRight:getComputedStyle(group).paddingRight,fullAnswerWidth:Math.abs(lineBox.right-groupBox.right)<1.1}});assert.deepEqual(answerEditLayout,{inHeading:true,noDirectButton:true,paddingRight:'0px',fullAnswerWidth:true});pass('official-answer edit control stays inside the 解答 heading without narrowing the answer');
  await fill.page.locator('.oaiEditBtn').click();const officialEditor=fill.page.locator('.oaiEditor'),officialText=officialEditor.locator('.oaiAnswerText');await officialText.fill('長い解答'.repeat(45)+'\n2行目の解答');await format(officialEditor,'official_answer.A','2行目','marker');const growState=await officialText.evaluate(ta=>({multiline:ta.tagName==='TEXTAREA'&&ta.value.includes('\n'),fits:ta.scrollWidth<=ta.clientWidth+1,grown:ta.getBoundingClientRect().height>42}));assert.deepEqual(growState,{multiline:true,fits:true,grown:true});await officialEditor.locator('.oaiSave').click();await officialEditor.waitFor({state:'detached'});await fill.page.locator('.fbAnswerLine strong .qbFmt-marker').waitFor();assert.ok((await fill.page.locator('.fbAnswerLine strong').last().textContent()).includes('\n'));assert.equal(await fill.page.evaluate(()=>testDB.question_occurrences[0].official_answer.A.includes('\n')),true);assert.equal(await fill.page.evaluate(()=>testDB.question_occurrences[0].official_answer_formatting.A.source_text),await fill.page.evaluate(()=>testDB.question_occurrences[0].official_answer.A));pass('official answers grow vertically, preserve line breaks and save formatting metadata');
