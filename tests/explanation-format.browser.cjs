...
async function run(browser,name){
  ...
  await save(ed);await page.locator('.qbChoiceCorrection .qbFmt-marker').waitFor();assert.equal(await page.evaluate(()=>testDB.choices[0].choice_text),'選択肢原文');assert.equal(await page.evaluate(()=>testDB.choices[0].is_correct),true);pass('choice explanations and all three detail fields format without changing option source or grading');
