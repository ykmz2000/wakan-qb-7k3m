...
async function run(browser,name){
  ...
  await p.locator('#next').click();await p.locator('.fbInput[data-k="A"]').fill('入力 <script>\n2行目');await p.locator('.fbInput[data-k="B"]').fill('Bの答え');const enteredA=await p.locator('.fbInput[data-k="A"]').inputValue();await p.locator('#answer').click();await ready(p);
  assert.equal(await p.locator('.qbAttemptCurrent .qbAttemptAnswer').textContent(),'自分の回答：A：'+enteredA.trim()+'\nB：Bの答え');assert.match(await p.locator('.qbAttemptCurrent').textContent(),/自動採点なし.*未評価/s);assert.equal(await p.locator('.qbAttemptCurrent script').count(),0);await rate(p,'○');
  ...
  await p.evaluate(()=>qbRetryCurrent());assert.equal(await p.locator('.fbInput[data-k="A"]').inputValue(),'','retry must start with blank written input');assert.equal(await p.locator('.fbInput[data-k="B"]').inputValue(),'','retry must start with blank written input');await p.locator('.fbInput[data-k="A"]').fill('別の答え');await p.locator('.fbInput[data-k="B"]').fill('次のB');await p.locator('#answer').click();await ready(p);assert.match(await p.locator('.qbAttemptPast').textContent(),/B：Bの答え.*自己評価：○/s);assert.equal(await p.locator('[data-qb-rate].on').count(),0,'new written attempt must not inherit previous rating');
  assert.deepEqual(errors,[]);await p.close();console.log(name+' PASS multiple choice, first attempt, multiple written fields, escaping and ungraded/unrated written attempts');
