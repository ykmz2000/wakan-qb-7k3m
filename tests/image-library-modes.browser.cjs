...
async function run(browser,name){
 ...
 assert.deepEqual(await p.evaluate(()=>db.qb_image_library_items[0].metadata.keywords),['眼球運動','lung cancer','保存前の入力']);await p.getByRole('button',{name:'一覧に戻る',exact:true}).click();assert.equal(await toggle.getAttribute('aria-pressed'),'false');
