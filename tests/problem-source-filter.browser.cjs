...
async function run(browser,name){
 ...
 assert.deepEqual(await selected(p),['q1','q2','q3','q4','q5','q6'],'initial selection includes every available year and unknown-year questions');
 ...
 await scope(p,'unit1');assert.equal(await p.locator('.problem').count(),5);assert.deepEqual(await selected(p),['q1','q2','q3','q4','q5']);
 ...
 await scope(p,'__all__');assert.equal(await p.locator('[data-source="year"][data-value="2010"]').count(),1);assert.deepEqual(await selected(p),['q1','q2','q3','q4','q5','q6','q7'],'default includes old and unknown-year data');
 ...
 await p.locator('[data-source="year"][data-value="2010"]').click();assert.equal((await selected(p)).includes('q7'),false,'clicking an active year removes it from the all-source default');
 ...
 await scope(p,'__all__');assert.deepEqual(await selected(p),['q1','q2','q3','q4','q5','q6','q7','year-2021','year-2022','year-2026','year-2027'],'default includes every past, future and unknown-year source');
 await p.clock.setFixedTime(new Date('2026-12-31T15:00:00Z'));await scope(p,'unit2');await scope(p,'__all__');assert.deepEqual(await selected(p),['q1','q2','q3','q4','q5','q6','q7','year-2021','year-2022','year-2026','year-2027'],'calendar boundary does not change the all-source default');
 ...
 assert.deepEqual(errors,[]);await p.close();console.log(name+' PASS all-year and unknown-year default, per-unit/all-unit scope, future data additions, manual selection, all/none, count and narrow screen');
}
