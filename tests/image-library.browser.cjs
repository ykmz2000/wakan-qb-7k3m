...
async function run(browser,name){
 ...
 await p.locator('.qbLibraryImageButton').first().click();await p.getByRole('button',{name:'編集',exact:true}).waitFor();await edit(p);await p.getByLabel('ファイル名',{exact:true}).waitFor();
 ...
 await p.getByRole('button',{name:'変更を保存',exact:true}).click();await p.waitForFunction(()=>db.qb_image_library_items[0].revision===2);await p.getByLabel('ファイル名',{exact:true}).waitFor();
 ...
 await p.getByLabel('ファイル名',{exact:true}).fill('競合中の下書き');await p.evaluate(()=>db.qb_image_library_items[0].revision++);await p.getByRole('button',{name:'変更を保存',exact:true}).click();await p.getByRole('button',{name:'最新情報を別表示'}).waitFor();
 assert.equal(await p.getByLabel('ファイル名',{exact:true}).inputValue(),'競合中の下書き');await p.getByRole('button',{name:'一覧に戻る'}).click();assert.equal(await p.getByLabel('ファイル名',{exact:true}).inputValue(),'競合中の下書き');assert.equal(await p.getByRole('searchbox').isVisible(),false);await p.getByLabel('ファイル名',{exact:true}).fill('眼球運動の総まとめ');await p.getByRole('button',{name:'一覧に戻る'}).click();pass('conflicting return keeps draft without overwriting; unchanged fields allow return');
 ...
 await p.locator('.qbLibraryEntry').click();await p.locator('.qbLibraryItem').first().waitFor();await p.locator('.qbLibraryImageButton').first().click();await p.getByRole('button',{name:'編集',exact:true}).waitFor();await edit(p);await p.getByLabel('ファイル名',{exact:true}).waitFor();
 ...
 await p.getByLabel('AIから受け取った結果',{exact:true}).fill(JSON.stringify(envelope));await p.getByRole('button',{name:'結果を取り込む',exact:true}).click();await p.getByLabel('ファイル名',{exact:true}).waitFor();await edit(p);await p.getByText('手動編集を保持したAIの変更案',{exact:true}).waitFor();
