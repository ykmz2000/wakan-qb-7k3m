...
async function run(browser,name){
 ...
 await p.evaluate(()=>{
  document.querySelector('.oeiActions').insertAdjacentHTML('afterbegin','<label class="oeiBtn">画像を選択<input type="file" hidden></label><button class="oeiBtn oeiPasteBtn">画像をコピペ</button><button class="oeiBtn oeiRecentBtn">最近の画像</button>');
 ...
 await p.getByRole('button',{name:'画像をコピペ',exact:true}).click();await p.getByRole('textbox',{name:'ライブラリへの画像貼り付け欄'}).waitFor();
 ...
 await p.getByRole('button',{name:'画像をコピペ',exact:true}).click();await p.waitForFunction(()=>db.qb_image_library_items.length===4);
 pass('clipboard button imports image bytes without typing into question editor');
 await p.getByRole('button',{name:'最近の画像',exact:true}).click();await p.locator('.qbripItem').first().waitFor();
 ...
 await p.getByRole('button',{name:'最近の画像',exact:true}).click();await p.locator('.qbripItem').first().waitFor();await p.locator('.qbripItem[data-id="r1"]').click();
 ...
 assert.equal(await p.evaluate(()=>db.qb_image_library_items.length),6);assert.equal(await p.getByRole('button',{name:'画像をコピペ',exact:true}).isEnabled(),true);pass('source download failure leaves no library record and restores controls');
