...
async function run(browser,label){
 ...
 await p.getByRole('button',{name:'一覧に戻る'}).click();await p.locator('.qbLibraryImageButton').nth(1).click();await p.getByRole('button',{name:'編集',exact:true}).waitFor();await edit(p);await p.getByLabel('ファイル名',{exact:true}).fill('本人が変更');await p.keyboard.press('Meta+s');await p.waitForFunction(()=>db.qb_image_library_items[1].revision===2);
 assert.equal(await p.getByLabel('ファイル名',{exact:true}).inputValue(),'本人が変更');
 ...
 const guarded=await p.getByLabel('ファイル名',{exact:true}).evaluate(t=>{
 ...
 await p.getByLabel('ファイル名',{exact:true}).press('s');assert.equal(await p.getByLabel('ファイル名',{exact:true}).inputValue(),'本人が変更s');
 // Non-cancellable native input is restored only for the exact insertion.
 const restored=await p.getByLabel('ファイル名',{exact:true}).evaluate(t=>{
