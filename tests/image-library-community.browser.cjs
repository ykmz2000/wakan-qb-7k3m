...
async function run(browser,label){
 ...
 assert.equal(await p.getByRole('button',{name:'画像を選択',exact:true}).count(),1);
 ...
 await p.locator('.qbLibraryImageButton').first().click();await p.getByText('投稿者のみ編集できます。',{exact:true}).waitFor();
 for(const name of ['変更を保存','トリミング','書き込み','ライブラリから削除'])assert.equal(await p.getByRole('button',{name,exact:true}).count(),0);
 assert.ok((await p.locator('.qbLibraryReadOnly').textContent()).includes('本文には動眼神経'));
 ...
 await p.getByRole('button',{name:'一覧へ戻る'}).click();await p.locator('.qbLibraryImageButton').nth(1).click();await p.getByLabel('画像名',{exact:true}).fill('本人が変更');await p.keyboard.press('Meta+s');await p.waitForFunction(()=>db.qb_image_library_items[1].revision===2);
