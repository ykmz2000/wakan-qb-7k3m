...
#qsoBar .qsoModeGroup{display:flex;gap:6px;flex:0 0 auto}#qsoBar .qsoMode{border:1px solid #cfd8e3;background:#fff;color:#536174;border-radius:9px;padding:8px 10px;font-weight:900}#qsoBar .qsoMode.on{background:#126fb3;color:#fff;border-color:#126fb3}
#qsoBar .qsoMsg{font-size:11px;color:#6f7786;margin-right:auto;line-height:1.45;min-width:160px}#qsoBar button{border-radius:9px;padding:8px 11px;font-weight:900}
...
.problem.qsoRow .qid{display:flex;align-items:center;gap:5px;min-width:0}.qsoNum{min-width:16px}.qsoHandle{display:none;touch-action:none;-webkit-user-select:none;user-select:none;border:0;background:#edf4f9;color:#126fb3;border-radius:8px;width:30px;height:34px;padding:0;font-size:21px;line-height:1;align-items:center;justify-content:center;cursor:grab;flex:0 0 auto}
body.qsoReorderMode .qsoHandle{display:flex}body.qsoReorderMode #view .problem.qsoRow{cursor:default!important}body.qsoReorderMode #view .problem.qsoRow:hover{background:inherit!important;outline:none!important}
...
#qsoUndo,#qsoSave{display:none}body.qsoReorderMode #qsoUndo,body.qsoReorderMode #qsoSave{display:inline-block}
@media(max-width:520px){.problem.qsoRow{grid-template-columns:58px 1fr}.qsoHandle{width:32px;height:36px}.problem.qsoRow .pick{grid-column:2}#qsoBar .qsoModeGroup{width:100%}#qsoBar .qsoMode{flex:1}}
`;
  document.head.appendChild(s)
}
function setGlobalMode(mode){
  const m=mode==='reorder'?'reorder':'open';window.QB_REORDER_MODE=m;window.QB_REORDER_ACTIVE=false;document.body.classList.toggle('qsoReorderMode',m==='reorder')
}
function clear(){
  drag=null;ctx=null;suppressClickRow=null;suppressClickUntil=0;window.QB_REORDER_ACTIVE=false;window.QB_REORDER_MODE='open';document.body.classList.remove('qsoReorderMode');document.getElementById('qsoBar')?.remove();
...
function updateBar(){
  ...
  document.querySelectorAll('#qsoBar .qsoMode').forEach(b=>{const on=b.dataset.mode===ctx.mode;b.classList.toggle('on',on);b.setAttribute('aria-pressed',String(on))});
  const msg=document.querySelector('#qsoBar .qsoMsg');
  if(msg&&!ctx.saving){
    if(ctx.mode==='open')msg.textContent='通常モード：問題を押すとその1問を開きます。';
    else msg.textContent=dirty?'並べ替え中です。保存するまでDBには反映されません。':'並べ替えモード：≡ をドラッグして順番を変更できます。';
  }
}
function restore(){
  if(!ctx)return;const rs=rows(),map=new Map(rs.map(r=>[rowId(r),r])),parent=rs[0]?.parentElement;if(!parent)return;
...
function setMode(mode,{force=false}={}){
  if(!ctx)return;const next=mode==='reorder'?'reorder':'open';if(next===ctx.mode)return;
  if(ctx.mode==='reorder'&&next==='open'&&ctx.dirty&&!force){
    const ok=window.confirm?.('保存していない並べ替えがあります。変更を破棄して通常モードに戻りますか？');
    if(ok===false)return;restore();
  }
  drag=null;document.querySelectorAll('.qsoDragging,.qsoDropTarget').forEach(x=>x.classList.remove('qsoDragging','qsoDropTarget'));
  ctx.mode=next;setGlobalMode(next);updateBar();
  window.dispatchEvent(new CustomEvent('qb-question-list-interaction-mode',{detail:{mode:next,unitId:ctx.unitId}}))
}
...
function onPointerDown(e){
  const h=e.currentTarget,r=h.closest('.problem');if(!r||!ctx||ctx.mode!=='reorder'||ctx.saving)return;
...
function onPointerMove(e){
  if(!drag||e.pointerId!==drag.pointerId||ctx?.mode!=='reorder')return;e.preventDefault();e.stopPropagation();drag.moved=true;window.QB_REORDER_ACTIVE=true;armClickSuppression(drag.row);
...
function suppressSyntheticClick(e){
  ...
  if(ctx?.mode==='reorder'||Date.now()<=suppressClickUntil){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation()}
}
async function save(){
  if(!ctx||ctx.mode!=='reorder'||ctx.saving)return;const current=idsNow();if(sameOrder(current,ctx.originalIds))return;
...
async function inject(){
  ...
  clear();ctx={unitId,originalIds:[...ids],rankValues,saving:false,dirty:false,mode:'open'};setGlobalMode('open');css();
  rs.forEach((row,i)=>{
  ...
  const listCard=rs[0].parentElement,bar=document.createElement('div');bar.id='qsoBar';bar.innerHTML='<div class="qsoModeGroup" role="group" aria-label="問題一覧の操作モード"><button class="qsoMode on" data-mode="open" type="button" aria-pressed="true">問題を開く</button><button class="qsoMode" data-mode="reorder" type="button" aria-pressed="false">並べ替え</button></div><div class="qsoMsg">通常モード：問題を押すとその1問を開きます。</div><button id="qsoUndo" type="button" disabled>元に戻す</button><button id="qsoSave" type="button" disabled>順番を保存</button>';
  listCard?.insertAdjacentElement('beforebegin',bar);
  bar.querySelectorAll('.qsoMode').forEach(b=>b.onclick=e=>{e.preventDefault();e.stopPropagation();setMode(b.dataset.mode)});
  document.getElementById('qsoUndo').onclick=restore;document.getElementById('qsoSave').onclick=save;renumber();updateBar()
}
function schedule(){clearTimeout(timer);timer=setTimeout(()=>inject().catch(console.error),80)}
function boot(){css();setGlobalMode('open');document.addEventListener('click',suppressSyntheticClick,true);['qb-screen-change','qb-app-ready','qb-question-order-updated'].forEach(ev=>window.addEventListener(ev,schedule));setTimeout(schedule,900)}
