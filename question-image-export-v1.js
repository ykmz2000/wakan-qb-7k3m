...
async function render(Q,options={}){
  ...
  const images=[];for(const row of r.data||[]){let blob;if(/^https?:\/\//i.test(String(row.image_path||''))){const response=await fetch(row.image_path);if(!response.ok)throw Error('問題画像の取得に失敗しました。通信状態を確認してください。');blob=await response.blob()}else{const file=await sb.storage.from('question-media').download(row.image_path);if(file.error)throw Error('問題画像の取得に失敗しました。通信状態を確認してください。');blob=file.data}let caption=row.caption||'';if(window.QBFiles?.isPDF(row.image_path)){const preview=await QBFiles.previewPage(blob);blob=preview.blob;caption=`PDFの1ページ目（全${preview.pages}ページ・全体はアプリで確認） ${caption}`}images.push({image:await imageFromBlob(blob),caption})}
