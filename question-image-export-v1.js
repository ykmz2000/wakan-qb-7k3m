...
async function render(Q,options={}){
  ...
  const r=await sb.from('question_images').select('image_path,storage_bucket,caption,alt_text,sort_order,created_at').eq('question_id',id).eq('placement','question').is('choice_id',null).order('sort_order').order('created_at');if(r.error)throw r.error;
  const images=[];for(const row of r.data||[]){const file=await sb.storage.from(row.storage_bucket||'question-media').download(row.image_path);if(file.error)throw Error('問題画像の取得に失敗しました。通信状態を確認してください。');let blob=file.data,caption=row.caption||'';if(window.QBFiles?.isPDF(row.image_path)){const preview=await QBFiles.previewPage(blob);blob=preview.blob;caption=`PDFの1ページ目（全${preview.pages}ページ・全体はアプリで確認） ${caption}`}images.push({image:await imageFromBlob(blob),caption})}
...
let outside=null;
function close(){if(!menu)return;menu.remove();menu=null;if(outside){document.removeEventListener('pointerdown',outside,true);outside=null}}
async function show(button){
  ...
  const save=document.createElement('button'),copy=document.createElement('button'),cancel=document.createElement('button');save.textContent='画像として保存';copy.textContent='画像としてコピー';cancel.textContent='閉じる';[save,copy,cancel].forEach(b=>b.type='button');save.disabled=copy.disabled=true;cancel.onclick=()=>{close();button.focus()};actions.append(save,copy,cancel);panel.append(title,info,qualityLabel,status,actions);document.body.append(panel);menu=panel;outside=e=>{if(menu===panel&&!panel.contains(e.target)&&e.target!==button){close();button.focus({preventScroll:true})}};document.addEventListener('pointerdown',outside,true);cancel.focus();
