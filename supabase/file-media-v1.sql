...
begin
 ...
 if p_copy_path is null or p_copy_path !~ '[.](png|jpg|jpeg|webp|gif|heic|heif|pdf)$' or p_copy_path not like p_question_id::text||'/'||p_placement||'/'||coalesce(p_choice_id::text,'question')||'/library-'||p_request_id::text||'.%' then raise exception '独立コピーの保存先が不正です';end if;
...
-- Keep visibility, limits, and existing types. Only add PDF support.
update storage.buckets set allowed_mime_types=array_append(allowed_mime_types,'application/pdf')
where id in ('question-media','qb-image-library','user-note-images')
and allowed_mime_types is not null and not ('application/pdf'=any(allowed_mime_types));
