drop function if exists public.qb_search_questions(text[],uuid,uuid[],integer);
...
returns table (
  ...
  question_occurrences jsonb, total_count bigint
)
...
as $$
  ...
  select m.id,m.subject_id,m.unit_id,m.stem,m.instruction,m.study_order,
    m.explanation_overview,m.examiner_intent,m.exam_summary,m.medical_verification_note,
    m.choices,m.question_occurrences,count(*) over() total_count
  from matched m
  order by m.study_order nulls last,m.id
  limit least(greatest(coalesce(p_limit,100),1),100)
  offset greatest(coalesce(p_offset,0),0);
$$;
revoke all on function public.qb_search_questions(text[],uuid,uuid[],integer,integer) from public,anon;
grant execute on function public.qb_search_questions(text[],uuid,uuid[],integer,integer) to authenticated,service_role;
