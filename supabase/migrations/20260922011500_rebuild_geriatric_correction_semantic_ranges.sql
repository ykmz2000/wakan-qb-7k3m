...
-- Rebuild learner-visible emphasis from the actual semantic replacements.
-- Each item is an independently meaningful changed value, term, direction or
-- condition.  Shared predicates, particles and text between two changes are
-- deliberately excluded.
with payload as (
  select *
  from jsonb_to_recordset($qb$[
  ...
  ]$qb$::jsonb) as x(id uuid,correction_text text,marks jsonb)
), corrected as (
  update public.choices c
  set correction_text=coalesce(p.correction_text,c.correction_text)
  from payload p
  where c.id=p.id
  returning c.id,c.correction_text,c.explanation_formatting
), ranges as (
  select
    p.id,
    c.correction_text,
...
set audit_fingerprint=public.qb_choice_correction_fingerprint(review.choice_id),
    reviewed_at=now(),
    rationale=review.rationale||' 2026-09-22：原文と訂正文を再比較し、共通語を除外して変更された意味単位だけへ太字・下線・アクセント色を設定。'
from applied a
