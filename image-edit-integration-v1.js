...
async function edit(c,rowId,restore=false,initialPage=1){if(opening||window.QBImageEditor.isOpen())return;opening=true;
  try{await window.QBImageStore.authorize(c);const row=await window.QBImageStore.get(c,rowId);
    ...
    const blob=await window.QBImageStore.download(c,row.image_path);return await (window.QBFiles?.isPDF(blob)?window.QBPDFEditor:window.QBImageEditor).open(blob,{title:'画像編集',initialPage,resetSource:row.original_image_path&&row.original_image_path!==row.image_path?()=>QBImageStore.download(c,row.original_image_path):null,onSave:async output=>{await window.QBImageStore.replace(c,row,output,'annotation');await Promise.resolve(c.onSaved?.()).catch(e=>console.warn("image refresh",e))}})
