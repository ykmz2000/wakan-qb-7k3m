...
async function edit(c,rowId,restore=false,initialPage=1,initialOrder=null){if(opening||window.QBImageEditor.isOpen())return;opening=true;
  try{await window.QBImageStore.authorize(c);const row=await window.QBImageStore.get(c,rowId);
    ...
    const blob=await window.QBImageStore.download(c,row.image_path);return await (window.QBFiles?.isPDF(blob)?window.QBPDFEditor:window.QBImageEditor).open(blob,{title:'画像編集',initialPage,initialOrder,resetSource:row.original_image_path&&row.original_image_path!==row.image_path?()=>QBImageStore.download(c,row.original_image_path):null,onSave:async output=>{await window.QBImageStore.replace(c,row,output,'annotation');await Promise.resolve(c.onSaved?.()).catch(e=>console.warn("image refresh",e))}})
...
async function actions(w,c,rowId,original){if(!rowId||!c||w.dataset.qbActionPending==='1'||w.querySelector(':scope > .qbImageWriteActions'))return;w.dataset.qbActionPending='1';try{await window.QBImageStore.authorize(c);if(!w.isConnected)return;w.querySelectorAll('.qsiCropTool,.oeiCrop').forEach(b=>b.remove());w.qbEditMedia=(page,order)=>edit(c,rowId,false,page,order);const a=document.createElement('div');a.className='qbImageWriteActions';for(const [label,run] of [[w.querySelector('[data-pdf-src]')?'PDF編集・トリミング':'画像編集',()=>edit(c,rowId)],...(!w.querySelector('.qsiDelete,.oeiDelete')?[['削除',()=>remove(c,rowId)]]:[])]){const b=document.createElement('button');b.type='button';b.textContent=label;b.onclick=e=>{e.stopPropagation();run()};a.append(b)}w.append(a);const del=w.querySelector('.qsiDelete,.oeiDelete');if(del)del.onclick=e=>{e.stopPropagation();remove(c,rowId)}}catch{}finally{delete w.dataset.qbActionPending}}
