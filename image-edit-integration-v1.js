...
async function edit(c,rowId,restore=false){if(opening||window.QBImageEditor.isOpen())return;opening=true;
  try{await window.QBImageStore.authorize(c);const row=await window.QBImageStore.get(c,rowId);
    if(restore){if(!row.original_image_path||row.original_image_path===row.image_path)return;if(!confirm('トリミング・画像編集を取り消して、元画像に戻してもよろしいですか？'))return;await window.QBImageStore.restore(c,row);await Promise.resolve(c.onSaved?.()).catch(e=>console.warn("image refresh",e));return}
    const blob=await window.QBImageStore.download(c,row.image_path);await window.QBImageEditor.open(blob,{title:'画像編集',onSave:async output=>{await window.QBImageStore.replace(c,row,output,'annotation');await Promise.resolve(c.onSaved?.()).catch(e=>console.warn("image refresh",e))}})
  }catch(e){alert('画像編集に失敗しました：'+(e.message||e))}finally{opening=false}
}
async function crop(c,rowId){if(opening||window.QBImageEditor.isOpen())return;opening=true;let url;
  try{await window.QBImageStore.authorize(c);const row=await window.QBImageStore.get(c,rowId),blob=await window.QBImageStore.download(c,row.image_path);url=URL.createObjectURL(blob);const output=await window.QBImageCrop.open(url,{title:'画像をトリミング'});if(output){await window.QBImageStore.replace(c,row,output);await c.onSaved?.()}}catch(e){alert('トリミングできませんでした：'+(e.message||e))}finally{if(url)URL.revokeObjectURL(url);opening=false}
}
...
async function actions(w,c,rowId,original){if(!rowId||!c||w.dataset.qbActionPending==='1'||w.querySelector(':scope > .qbImageWriteActions'))return;w.dataset.qbActionPending='1';try{await window.QBImageStore.authorize(c);if(!w.isConnected)return;const a=document.createElement('div');a.className='qbImageWriteActions';for(const [label,run] of [...(!w.querySelector('[data-pdf-src]')?[['画像編集',()=>edit(c,rowId)]]:[]),...(!w.querySelector('.qsiDelete,.oeiDelete')?[['削除',()=>remove(c,rowId)]]:[]),...(original&&!w.querySelector('[data-pdf-src]')?[['元画像に戻す',()=>edit(c,rowId,true)]]:[])]){const b=document.createElement('button');b.type='button';b.textContent=label;b.onclick=e=>{e.stopPropagation();run()};a.append(b)}w.append(a);const del=w.querySelector('.qsiDelete,.oeiDelete');if(del)del.onclick=e=>{e.stopPropagation();remove(c,rowId)}}catch{}finally{delete w.dataset.qbActionPending}}
