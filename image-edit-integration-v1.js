...
async function actions(w,c,rowId,original){if(!rowId||!c||w.dataset.qbActionPending==='1'||w.querySelector(':scope > .qbImageWriteActions'))return;w.dataset.qbActionPending='1';try{await window.QBImageStore.authorize(c);if(!w.isConnected)return;const a=document.createElement('div');a.className='qbImageWriteActions';for(const [label,run] of [...(w.classList.contains('qsiImgWrap')?[]:[['トリミング',()=>crop(c,rowId)]]),['書き込み',()=>edit(c,rowId)],...(original?[['元画像に戻す',()=>edit(c,rowId,true)]]:[])]){const b=document.createElement('button');b.type='button';b.textContent=label;b.onclick=e=>{e.stopPropagation();run()};a.append(b)}w.append(a)}catch{}finally{delete w.dataset.qbActionPending}}
