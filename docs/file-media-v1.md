...
Photo long-press, stationary two-finger tap, or context click opens Delete/Copy/Duplicate/Save. Duplicate clones the complete editable object (source asset, crop, position, size and transform) under a new ID. The copies can then be edited or deleted independently. The same stationary gestures on empty canvas open Paste/Add; moving cancels either menu gesture. Copy retains source and crop metadata, with a local paste fallback if system clipboard reading/writing is unavailable. Saving writes native-resolution PNG plus editable metadata.
