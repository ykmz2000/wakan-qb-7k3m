...
function close(){if(!active)return;const state=active,{dialog,origin,key}=state;active=null;state.cancelled=true;document.removeEventListener('keydown',key,true);dialog.remove();if(origin?.isConnected)origin.focus?.({preventScroll:true})}
...
function open(options={}){
 ...
 const key=e=>{if(e.key==='Escape'){e.preventDefault();e.stopImmediatePropagation();close()}if(e.key==='Tab'){const buttons=[...sheet.querySelectorAll('button:not(:disabled)')],i=buttons.indexOf(document.activeElement);if(!buttons.length)return;e.preventDefault();buttons[(i+(e.shiftKey?-1:1)+buttons.length)%buttons.length]?.focus()}};const state={dialog,origin,key,cancelled:false,prepared:new Map()};active=state;document.addEventListener('keydown',key,true);document.body.append(dialog);
 const actionButtons=[...sheet.querySelectorAll('[data-action]')];actionButtons.forEach(button=>button.disabled=true);status.textContent='書き出しを準備中…';for(const format of ['image','pdf'])output(options,format).then(result=>{if(active!==state||state.cancelled)return;const name=safeName(inferredName(options.source,options.name)+'.'+result.extension,'書き出し.'+result.extension);state.prepared.set(format,{...result,name});actionButtons.filter(button=>button.dataset.format===format).forEach(button=>button.disabled=false);if(state.prepared.size===2)status.textContent=''}).catch(error=>{if(active!==state||state.cancelled)return;status.textContent=`${format==='pdf'?'PDF':'画像'}を準備できませんでした：${error?.message||error}`});
 dismiss.onclick=close;dialog.addEventListener('click',e=>{if(e.target===dialog)close()});sheet.addEventListener('click',async e=>{const button=e.target.closest('[data-action]');if(!button||button.disabled||active!==state)return;e.preventDefault();const format=button.dataset.format,action=button.dataset.action,result=state.prepared.get(format);if(!result)return;const all=[...sheet.querySelectorAll('button')],label=button.textContent;all.forEach(b=>b.disabled=true);status.textContent='';try{if(action==='copy'){const operation=copy(result.blob);await operation;if(active===state)status.textContent=`${format==='pdf'?'PDF':'画像'}をコピーしました。`}else if(action==='save'){download(result.blob,result.name);if(active===state)status.textContent=`${result.name} を保存しました。`}else{const operation=nativeShare(result.blob,result.name);await operation;if(active===state)status.textContent='共有先を開きました。'}}catch(error){if(active===state&&error?.name!=='AbortError')status.textContent=error?.message||'書き出しできませんでした。'}finally{if(active===state){button.textContent=label;all.forEach(b=>b.disabled=b.dataset.format&&!state.prepared.has(b.dataset.format))}}});dismiss.focus({preventScroll:true});
}
...
function position(button,host,media){if(!button.isConnected||!media.isConnected)return;const hr=host.getBoundingClientRect(),mr=media.getBoundingClientRect();if(!hr.width||!mr.width)return;button.style.left=Math.max(4,mr.right-hr.left-46)+'px';button.style.top=Math.max(4,mr.bottom-hr.top-46)+'px'}
function decorate(node){
 if(!(node instanceof Element)||node.closest('.qbImageLightbox,.qbPdfModal,.qbMediaShareDialog,.qbQuestionPreviewPanel'))return;
 if(node.matches('.qbPdfInlinePage')){node.querySelector(':scope > .qbMediaShareButton')?.remove();const card=node.closest('.qbPdfCard');if(card)decorate(card);return}const host=hostFor(node);if(!host||host.querySelector(':scope > .qbMediaShareButton'))return;const options=mediaOptions(node);if(!options?.source)return;
 const button=el('button','qbMediaShareButton');button.type='button';button.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 15V3m0 0 4 4m-4-4L8 7M5 11v8h14v-8"/></svg>';button.setAttribute('aria-label','書き出し・共有');button.title='書き出し・共有';host.classList.add('qbMediaShareHost');host.append(button);const update=()=>position(button,host,node);requestAnimationFrame(update);if(node.matches('img')&&!node.complete)node.addEventListener('load',update,{once:true});if(window.ResizeObserver)new ResizeObserver(update).observe(node);else window.addEventListener('resize',update,{passive:true});
