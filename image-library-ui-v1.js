...
const SELECT='id,subject_id,unit_id,stem,instruction,study_order,explanation_overview,examiner_intent,exam_summary,medical_verification_note,choices(choice_key,choice_text,is_correct,explanation,correction_text,correct_for_other_context,examiner_distinction),question_occurrences(academic_year,exam_type,original_question_number,official_answer)';
...
async function questionRows(sb,subjectId,allowedSubjectIds){
 const key=subjectId||'*'+allowedSubjectIds.join(',');if(questionCache.has(key))return questionCache.get(key);const all=[];
 for(let start=0;;start+=500){let q=sb.from('questions').select(SELECT).eq('status','published').order('study_order',{ascending:true}).range(start,start+499);if(subjectId)q=q.eq('subject_id',subjectId);else if(allowedSubjectIds.length)q=q.in('subject_id',allowedSubjectIds);const r=await q;if(r.error)throw r.error;all.push(...(r.data||[]));if((r.data||[]).length<500)break}
 questionCache.set(key,all);return all;
}
function addQuestionSearch(overlay){
 ...
 fileBody.before(problemBody);const form=problemBody.querySelector('form'),subject=form.querySelector('select'),input=form.querySelector('input'),status=problemBody.querySelector('[role=status]'),results=problemBody.querySelector('.qbLibraryQuestionResults'),close=header.querySelector('button');
 const subjects=(window.qbGetLibrarySubjects?.()||[]).slice().sort((a,b)=>(a.sort_order||0)-(b.sort_order||0));subjects.forEach(s=>subject.add(new Option(s.name,s.id)));const names=new Map(subjects.map(s=>[String(s.id),s.name]));let generation=0,timer=0;
 async function search(){const query=input.value.trim(),version=++generation;results.replaceChildren();if(!query){status.textContent='検索語を入力してください。';return}status.textContent='問題を検索中…';try{const all=await questionRows(window.qbSupabase,subject.value,subjects.map(s=>s.id)),found=all.map(q=>({q,hit:questionMatches(q,query)})).filter(x=>x.hit.length).slice(0,100);if(version!==generation||!problemBody.isConnected)return;status.textContent=found.length?`${found.length}問を表示${found.length===100?'（先頭100問）':''}`:'該当する問題はありません。';for(const {q,hit} of found){const o=(q.question_occurrences||[])[0]||{},button=document.createElement('button');button.type='button';button.className='qbLibraryQuestionCard';button.innerHTML=`<strong>${escapeHTML(q.stem||'問題文未登録')}</strong><span>${escapeHTML(names.get(String(q.subject_id))||'科目不明')}${o.academic_year?' · '+escapeHTML(o.academic_year):''}${o.original_question_number?' · '+escapeHTML(o.original_question_number):''}</span><small>${escapeHTML(hit.join('・'))}で一致</small><p>${escapeHTML(questionSnippet(q,query))}</p>`;button.onclick=()=>{status.textContent='問題を開いています…';close?.click();setTimeout(()=>window.qbOpenLibraryQuestion({questionId:q.id,subjectId:q.subject_id,targetUnitId:q.unit_id}).catch(e=>alert(e.message||e)),0)};results.append(button)}}catch(e){if(version===generation)status.textContent='検索できませんでした：'+(e.message||e)}}
 form.onsubmit=e=>{e.preventDefault();clearTimeout(timer);search()};input.oninput=()=>{clearTimeout(timer);timer=setTimeout(search,300)};subject.onchange=()=>{if(input.value.trim())search()};
 function selectTab(kind){const problem=kind==='problem';problemTab.setAttribute('aria-selected',String(problem));fileTab.setAttribute('aria-selected',String(!problem));problemTab.tabIndex=problem?0:-1;fileTab.tabIndex=problem?-1:0;problemBody.hidden=!problem;fileBody.hidden=problem;if(footer)footer.hidden=problem;(problem?input:fileBody.querySelector('input,button'))?.focus({preventScroll:true})}
...
window.QBQuestionLibrarySearch={matches:questionMatches,clearCache:()=>questionCache.clear()};
})();
