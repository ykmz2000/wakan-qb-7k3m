 ...
 return new Promise(resolve=>{
  ...
  function uploadMode(values){
   ...
   return new Promise(resolve=>{
    ...
    function drawOrder(){previews.replaceChildren();values.forEach((value,index)=>{const row=el('div','qbLibraryUploadPreview'),name=value.name||value.metadata?.name||`ファイル ${index+1}`,im=el('img');im.alt=name;row.append(im,el('span','',`${index+1}. ${name}`));if(value instanceof Blob){const url=URL.createObjectURL(value);urls.push(url);if(window.QBFiles?.isPDF(value)){const holder=el('span');holder.innerHTML=QBFiles.markup(url,name);const card=holder.firstElementChild;card.dataset.pdfPassive='true';im.replaceWith(card)}else im.src=url}else if(value.object_path)S.signedURL(sb,value.object_path).then(url=>{if(im.isConnected)media(im,url)}).catch(()=>im.remove());else if(value.image_path){const url=value.image_url||value.url||(/^https?:\/\//i.test(String(value.image_path||''))?String(value.image_path):sb.storage.from('question-media').getPublicUrl(value.image_path).data.publicUrl);media(im,url)};const move=delta=>{const next=index+delta;if(next<0||next>=values.length)return;[values[index],values[next]]=[values[next],values[index]];drawOrder()};const up=btn('↑',()=>move(-1)),down=btn('↓',()=>move(1));up.setAttribute('aria-label',`${name}を前へ`);down.setAttribute('aria-label',`${name}を後ろへ`);up.disabled=index===0;down.disabled=index===values.length-1;row.append(up,down);previews.append(row)})}drawOrder();
  ...
  async function drawDetail(initial,note,editing){
   let row=initial,m=row.metadata||{};const img=el('img','qbLibraryDetailImage');img.alt=m.name||'ライブラリ画像';zoomable(img);if(owns(row))img.qbEditMedia=async(initialPage=1,initialOrder=null)=>{if(dirty()){note.textContent='先に情報の変更を保存してください。';return}setBusy(true);let output;try{const blob=await S.download(sb,row);output=await (pdf(row)?QBPDFEditor:QBImageEditor).open(blob,{initialPage,initialOrder,resetSource:row.original_path&&row.original_path!==row.object_path?()=>S.download(sb,{...row,object_path:row.original_path}):null,onSave:async output=>{row=await S.replace(sb,row,output)}});await load(true)}finally{setBusy(false)}await showDetail(row,editing);return output};const updated=updatedLine(row);detailView.append(authorLine(row),updated,img);S.signedURL(sb,row.object_path).then(url=>{if(img.isConnected)media(img,url)}).catch(e=>report(note,e));
   if(!editing||!owns(row)){
  ...
  function carousel(members,onOpen,version,zoom=false){
   ...
   for(const row of members){const b=btn('',()=>onOpen(row),'qbLibrarySlide'),im=el('img');im.loading='lazy';im.alt=row.metadata?.name||'画像';if(zoom){zoomable(im);b.tabIndex=-1;b.dataset.row=row.id;if(owns(row))b.qbEditMedia=async(initialPage=1,initialOrder=null)=>{if(busy||dirty())return;setBusy(true);try{const latest=await S.get(sb,row.id);if(!owns(latest))throw Error('このファイルを編集する権限がありません。');const output=await (pdf(latest)?QBPDFEditor:QBImageEditor).open(await S.download(sb,latest),{initialPage,initialOrder,resetSource:latest.original_path&&latest.original_path!==latest.object_path?()=>S.download(sb,{...latest,object_path:latest.original_path}):null,onSave:async value=>Object.assign(row,await S.replace(sb,latest,value))});if(output instanceof Blob){if(b.isConnected)media(im,await S.signedURL(sb,row.object_path));await load(true)}return output}finally{setBusy(false)}}}b.append(im);track.append(b);S.signedURL(sb,row.object_path).then(url=>{if(im.isConnected&&!closed)media(im,url)}).catch(()=>im.alt='画像を読み込めませんでした')}
  ...
  async function loadFileLibrary(){
   if(loadFileLibrary.started||closed)return;loadFileLibrary.started=true;
   try{[catalogRows,terms]=await Promise.all([S.taxonomy(sb),S.catalog(sb,'qb_image_library_terms')]);if(closed)return;refreshCatalogFilters();await load(true)}catch(e){report(status,e)}
  }
  overlay.focus();
...
/* The top-level library opens on question search; attachment pickers stay on files. */
(()=>{
'use strict';
...
function questionFields(q){
 const choices=q.choices||[],occ=q.question_occurrences||[];
 return{
  '問題文':[q.stem,q.instruction],
  '選択肢':choices.flatMap(c=>[c.choice_key,c.choice_text]),
  '解答':[...choices.filter(c=>c.is_correct).flatMap(c=>[c.choice_key,c.choice_text]),...occ.map(o=>flatten(o.official_answer))],
  '解説':[q.explanation_overview,q.examiner_intent,q.exam_summary,q.medical_verification_note,...choices.flatMap(c=>[c.explanation,c.correction_text,c.correct_for_other_context,c.examiner_distinction])]
 };
}
function questionMatches(q,query){const tokens=normalize(query).split(' ').filter(Boolean),groups=questionFields(q),joined=normalize(Object.values(groups).flat().map(flatten).join(' '));return tokens.length&&tokens.every(token=>joined.includes(token))?Object.entries(groups).filter(([,values])=>tokens.some(token=>normalize(values.map(flatten).join(' ')).includes(token))).map(([name])=>name):[]}
function questionSnippet(q,query){const token=normalize(query).split(' ').find(Boolean)||'',values=Object.values(questionFields(q)).flat().map(flatten).filter(Boolean),value=values.find(v=>normalize(v).includes(token))||q.stem||'';return String(value).replace(/\s+/g,' ').slice(0,150)}
async function questionRows(sb,query,subjectId,allowedSubjectIds,offset=0){
 const tokens=normalize(query).split(' ').filter(Boolean);if(!tokens.length)return[];
 const r=await sb.rpc('qb_search_question_cards',{p_tokens:tokens,p_subject_id:subjectId||null,p_allowed_subject_ids:allowedSubjectIds,p_limit:100,p_offset:offset});
 if(r.error)throw r.error;return r.data||[];
}
function addQuestionSearch(overlay){
 if(overlay.dataset.qbQuestionSearch||!window.qbOpenLibraryQuestion||overlay.getAttribute('aria-label')!=='ライブラリ')return;overlay.dataset.qbQuestionSearch='1';
 const panel=overlay.querySelector(':scope > .qbLibraryPanel'),fileBody=panel?.querySelector(':scope > .qbLibraryBody'),header=panel?.querySelector(':scope > .qbLibraryHeader'),footer=panel?.querySelector(':scope > .qbLibraryFooter');if(!panel||!fileBody||!header)return;
 if(!document.getElementById('qbLibraryQuestionCss')){const style=document.createElement('style');style.id='qbLibraryQuestionCss';style.textContent='.qbLibraryTabs{display:grid;grid-template-columns:1fr 1fr;border-bottom:1px solid var(--line);padding:0 18px}.qbLibraryPanel .qbLibraryTabs button{border:0;border-bottom:3px solid transparent;color:var(--muted);background:transparent}.qbLibraryPanel .qbLibraryTabs button[aria-selected=true]{border-bottom-color:var(--accent);color:var(--accent)}.qbLibraryQuestionSearch{display:grid;grid-template-columns:minmax(150px,.38fr) minmax(240px,1fr) auto;gap:8px;align-items:end;padding:14px 0}.qbLibraryQuestionSearch label>span{display:block;margin-bottom:4px;color:var(--muted);font-size:12px;font-weight:700}.qbLibraryQuestionSearch :is(input,select){width:100%}.qbLibraryQuestionResults{display:grid;gap:10px}.qbLibraryPanel button.qbLibraryQuestionCard{display:grid;gap:5px;width:100%;padding:14px;text-align:left;color:var(--text);border-color:var(--line);background:var(--card)}.qbLibraryQuestionCard strong{line-height:1.55}.qbLibraryQuestionCard span,.qbLibraryQuestionCard small{color:var(--muted);font-size:12px}.qbLibraryQuestionCard small{color:var(--accent);font-weight:800}.qbLibraryQuestionCard p{margin:2px 0 0;color:var(--text);font-size:14px;line-height:1.55;font-weight:400;white-space:normal;overflow-wrap:anywhere}@media(max-width:600px){.qbLibraryTabs{padding:0 12px}.qbLibraryQuestionSearch{grid-template-columns:1fr auto}.qbLibraryQuestionSearch label:first-child{grid-column:1/-1}}';document.head.append(style)}
 const tabs=document.createElement('div');tabs.className='qbLibraryTabs';tabs.setAttribute('role','tablist');tabs.setAttribute('aria-label','ライブラリ検索');
 const problemTab=document.createElement('button'),fileTab=document.createElement('button');problemTab.type=fileTab.type='button';problemTab.textContent='問題';fileTab.textContent='ファイル';problemTab.setAttribute('role','tab');fileTab.setAttribute('role','tab');tabs.append(problemTab,fileTab);header.after(tabs);
 const problemBody=document.createElement('div');problemBody.className='qbLibraryBody qbLibraryQuestionBody';problemBody.innerHTML='<form class="qbLibraryQuestionSearch"><label><span>科目</span><select aria-label="問題の科目"><option value="">すべての科目</option></select></label><label><span>検索語</span><input type="search" aria-label="問題を検索" placeholder="問題文・選択肢・解答・解説を検索"></label><button type="submit">検索</button></form><div class="qbLibraryStatus" role="status">検索語を入力してください。</div><div class="qbLibraryQuestionResults"></div><button type="button" class="qbLibraryQuestionMore" hidden>続きを表示</button>';
 fileBody.before(problemBody);const form=problemBody.querySelector('form'),subject=form.querySelector('select'),input=form.querySelector('input'),status=problemBody.querySelector('[role=status]'),results=problemBody.querySelector('.qbLibraryQuestionResults'),more=problemBody.querySelector('.qbLibraryQuestionMore'),close=header.querySelector('button');
 const subjects=(window.qbGetLibrarySubjects?.()||[]).slice().sort((a,b)=>(a.sort_order||0)-(b.sort_order||0));subjects.forEach(s=>subject.add(new Option(s.name,s.id)));const names=new Map(subjects.map(s=>[String(s.id),s.name]));let generation=0,timer=0,composing=false;
 async function search(append=false){const query=input.value.trim(),version=append?generation:++generation,offset=append?results.childElementCount:0;if(!append){results.replaceChildren();more.hidden=true}if(!query){status.textContent='検索語を入力してください。';return}status.textContent=append?'続きを読み込み中…':'問題を検索中…';more.disabled=true;try{const rows=await questionRows(window.qbSupabase,query,subject.value,subjects.map(s=>s.id),offset);if(version!==generation||!problemBody.isConnected)return;for(const q of rows){const button=document.createElement('button'),hit=Array.isArray(q.hit_kinds)?q.hit_kinds:[];button.type='button';button.className='qbLibraryQuestionCard';button.innerHTML=`<strong>${escapeHTML(q.stem||'問題文未登録')}</strong><span>${escapeHTML(names.get(String(q.subject_id))||'科目不明')}${q.academic_year?' · '+escapeHTML(q.academic_year):''}${q.original_question_number?' · '+escapeHTML(q.original_question_number):''}</span><small>${escapeHTML(hit.join('・'))}で一致</small><p>${escapeHTML(q.snippet||q.stem||'')}</p>`;button.onclick=()=>{status.textContent='問題を開いています…';close?.click();setTimeout(()=>window.qbOpenLibraryQuestion({questionId:q.id,subjectId:q.subject_id,targetUnitId:q.unit_id}).catch(e=>alert(e.message||e)),0)};results.append(button)}const total=Number(rows[0]?.total_count||0),shown=results.childElementCount;status.textContent=total?`${shown} / ${total}問を表示`:'該当する問題はありません。';more.hidden=shown>=total;more.disabled=false}catch(e){if(version===generation){status.textContent='検索できませんでした：'+(e.message||e);more.hidden=true;more.disabled=false}}}
 form.onsubmit=e=>{e.preventDefault();clearTimeout(timer);if(!composing)search()};
 input.addEventListener('compositionstart',()=>{composing=true;clearTimeout(timer)});
 input.addEventListener('compositionend',()=>{composing=false;clearTimeout(timer);timer=setTimeout(search,300)});
 input.oninput=e=>{clearTimeout(timer);if(!composing&&!e.isComposing)timer=setTimeout(search,300)};subject.onchange=()=>{if(input.value.trim())search()};more.onclick=()=>search(true);
 function selectTab(kind){const problem=kind==='problem';problemTab.setAttribute('aria-selected',String(problem));fileTab.setAttribute('aria-selected',String(!problem));problemTab.tabIndex=problem?0:-1;fileTab.tabIndex=problem?-1:0;problemBody.hidden=!problem;fileBody.hidden=problem;if(footer)footer.hidden=problem;if(!problem)overlay.dispatchEvent(new CustomEvent('qb-library-files-requested'));(problem?input:fileBody.querySelector('input,button'))?.focus({preventScroll:true})}
 close?.addEventListener('click',()=>{if(problemBody.hidden)return;problemBody.hidden=true;fileBody.hidden=false;if(footer)footer.hidden=false},true);
 overlay.addEventListener('keydown',e=>{if(e.key==='Escape'&&!problemBody.hidden){problemBody.hidden=true;fileBody.hidden=false;if(footer)footer.hidden=false}},true);
 problemTab.onclick=()=>selectTab('problem');fileTab.onclick=()=>selectTab('file');for(const tab of [problemTab,fileTab])tab.onkeydown=e=>{if(!['ArrowLeft','ArrowRight'].includes(e.key))return;e.preventDefault();(tab===problemTab?fileTab:problemTab).click()};selectTab('problem');
}
const questionObserver=new MutationObserver(records=>{for(const record of records)for(const node of record.addedNodes)if(node.nodeType===1){if(node.matches?.('.qbLibraryOverlay'))addQuestionSearch(node);node.querySelectorAll?.('.qbLibraryOverlay').forEach(addQuestionSearch)}});questionObserver.observe(document.body,{childList:true,subtree:true});document.querySelectorAll('.qbLibraryOverlay').forEach(addQuestionSearch);
window.QBQuestionLibrarySearch={matches:questionMatches};
})();
