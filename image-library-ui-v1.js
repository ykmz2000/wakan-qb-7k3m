...
async function questionRows(sb,query,subjectId,allowedSubjectIds,offset=0){
 ...
 const r=await sb.rpc('qb_search_questions',{p_tokens:tokens,p_subject_id:subjectId||null,p_allowed_subject_ids:allowedSubjectIds,p_limit:100,p_offset:offset});
...
function addQuestionSearch(overlay){
 ...
 async function search(append=false){const query=input.value.trim(),version=append?generation:++generation,offset=append?results.childElementCount:0;if(!append){results.replaceChildren();more.hidden=true}if(!query){status.textContent='検索語を入力してください。';return}status.textContent=append?'続きを読み込み中…':'問題を検索中…';more.disabled=true;try{const rows=await questionRows(window.qbSupabase,query,subject.value,subjects.map(s=>s.id),offset),found=rows.map(q=>({q,hit:questionMatches(q,query)})).filter(x=>x.hit.length);if(version!==generation||!problemBody.isConnected)return;for(const {q,hit} of found){const o=(q.question_occurrences||[])[0]||{},button=document.createElement('button');button.type='button';button.className='qbLibraryQuestionCard';button.innerHTML=`<strong>${escapeHTML(q.stem||'問題文未登録')}</strong><span>${escapeHTML(names.get(String(q.subject_id))||'科目不明')}${o.academic_year?' · '+escapeHTML(o.academic_year):''}${o.original_question_number?' · '+escapeHTML(o.original_question_number):''}</span><small>${escapeHTML(hit.join('・'))}で一致</small><p>${escapeHTML(questionSnippet(q,query))}</p>`;button.onclick=()=>{status.textContent='問題を開いています…';close?.click();setTimeout(()=>window.qbOpenLibraryQuestion({questionId:q.id,subjectId:q.subject_id,targetUnitId:q.unit_id}).catch(e=>alert(e.message||e)),0)};results.append(button)}const total=Number(rows[0]?.total_count||0),shown=results.childElementCount;status.textContent=total?`${shown} / ${total}問を表示`:'該当する問題はありません。';more.hidden=shown>=total;more.disabled=false}catch(e){if(version===generation){status.textContent='検索できませんでした：'+(e.message||e);more.hidden=true;more.disabled=false}}}
 form.onsubmit=e=>{e.preventDefault();clearTimeout(timer);search()};input.oninput=()=>{clearTimeout(timer);timer=setTimeout(search,300)};subject.onchange=()=>{if(input.value.trim())search()};more.onclick=()=>search(true);
