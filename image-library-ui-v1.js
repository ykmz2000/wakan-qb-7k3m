...
async function open({context=null}={}){
 ...
 return new Promise(resolve=>{
  ...
  function metadataForm(form,m){
   ...
   function field(key,parent){const f=inputField(C.LABELS[key],fieldText(m[key]),['notes','visual_summary','ocr_text'].includes(key));f.host.classList.add('qbLibraryWide');if(key==='roles')f.input.placeholder=ROLES.join('、');if(key==='aspects')f.input.placeholder=ASPECTS.join('、');if(key==='ocr_text')f.input.classList.add('qbLibraryTranscript');inputs[key]=f.input;controls[key]=()=>C.ARRAY_FIELDS.includes(key)?C.list(f.input.value):f.input.value;parent.append(f.host)}
