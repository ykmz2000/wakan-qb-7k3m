...
const IMAGE_TYPE=/^image\/(?:avif|bmp|gif|heic|heif|jpeg|png|svg\+xml|tiff|webp)$/;
...
async function write(preview,record){
 const blobs=[],assets={};let offset=0;for(const [id,blob] of record.assets){if(!IMAGE_TYPE.test(blob.type))throw Error('元画像の形式が不正です。');assets[id]={offset,length:blob.size,type:blob.type};blobs.push(blob);offset+=blob.size;}
...
const dataURL=blob=>new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(r.result);r.onerror=()=>reject(Error('編集素材を読み込めませんでした。'));r.readAsDataURL(blob)});
...
async function fromLegacy(record){const assets=new Map();for(const [id,url] of Object.entries(record.assets||{})){const type=/^data:([^;,]+);base64,/.exec(url)?.[1]||'';if(!IMAGE_TYPE.test(type))throw Error('編集素材の形式が不正です。');assets.set(id,new Blob([await(await fetch(url)).arrayBuffer()],{type}))}const scene=copy(record.scene);scene.base=null;return{scene,assets}}
...
async function drawPDF(pdf,page,record,{overlay=false,depth=0}={}){
 ...
 const raster=async(blob)=>{if(['image/png','image/jpeg'].includes(blob.type)){const embedded=blob.type==='image/jpeg'?await pdf.embedJpg(await blob.arrayBuffer()):await pdf.embedPng(await blob.arrayBuffer());return{image:embedded,w:embedded.width,h:embedded.height}}const a=await image(blob);try{const w=a.img.naturalWidth||a.img.width,h=a.img.naturalHeight||a.img.height,pixels=w*h,scale=Math.min(1,8192/Math.max(w,h),Math.sqrt(32000000/pixels)),rw=Math.max(1,Math.round(w*scale)),rh=Math.max(1,Math.round(h*scale)),png=await QBImageEditor.encodePng(rw,rh,ctx=>ctx.drawImage(a.img,0,0,rw,rh));return{image:await pdf.embedPng(await png.arrayBuffer()),w,h}}finally{URL.revokeObjectURL(a.url);a.img.close?.()}};
 async function asset(i){const blob=record.assets.get(i.assetId);if(!blob)throw Error('PDFに保存する元画像がありません。');let a=loaded.get(i.assetId);if(!a){const nested=await read(blob);a=nested?{nested,w:nested.scene.width,h:nested.scene.height}:await raster(blob);loaded.set(i.assetId,a)}const r=i.sourceRect||{x:0,y:0,w:a.w,h:a.h};push(L.pushGraphicsState());clip(i.x,i.y,i.w,i.h);push(matrix(i.w/r.w,0,0,i.h/r.h,i.x-r.x*i.w/r.w,i.y-r.y*i.h/r.h));if(a.nested)await drawPDF(pdf,page,a.nested,{depth:depth+1});else{push(matrix(1,0,0,-1,0,a.h));page.drawImage(a.image,{x:0,y:0,width:a.w,height:a.h})}push(L.popGraphicsState())}
