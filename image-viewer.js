...
function previewChoices(question){
  const choices=Array.isArray(question?.choices)?question.choices:[];if(!choices.length)return null;
  const list=document.createElement('ol');list.className='qbQuestionPreviewChoices';list.setAttribute('aria-label','選択肢');
  for(let index=0;index<choices.length;index++){
    const choice=choices[index]||{},row=document.createElement('li'),key=document.createElement('span'),text=document.createElement('span');
    row.className='qbQuestionPreviewChoiceOption';key.className='qbQuestionPreviewChoiceKey';key.textContent=String(choice.choice_key||index+1);text.className='qbQuestionPreviewChoiceText';text.textContent=String(choice.choice_text||'');row.append(key,text);list.append(row);
  }
  return list;
}
...
function attachQuestionPreview(root,toolbar,{before=null}={}){
  ...
  const choices=previewChoices(data.question);if(choices)problem.append(choices);
...
.qbQuestionPreviewPanel{position:fixed;z-index:10550;top:max(66px,calc(env(safe-area-inset-top) + 56px));right:max(12px,env(safe-area-inset-right));box-sizing:border-box;width:min(440px,calc(100vw - max(24px,env(safe-area-inset-left) + env(safe-area-inset-right) + 24px)));max-width:calc(100vw - max(24px,env(safe-area-inset-left) + env(safe-area-inset-right) + 24px));max-height:min(62dvh,560px);overflow-x:hidden;overflow-y:auto;padding:14px;background:var(--card,#fff);color:var(--text,#172033);border:1px solid var(--line,#dce3ec);border-radius:14px;box-shadow:0 14px 48px #0008;text-align:left;touch-action:pan-y;overscroll-behavior:contain}.qbQuestionPreviewProblem,.qbQuestionPreviewAnswer{min-width:0;max-width:100%;overflow-x:hidden}
...
.qbQuestionPreviewStem{max-width:100%;font-size:15px;font-weight:700;line-height:1.6;white-space:pre-wrap;overflow-wrap:anywhere}.qbQuestionPreviewMedia{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(96px,100%),1fr));gap:8px;min-width:0;max-width:100%;margin-top:12px;overflow:hidden}.qbQuestionPreviewMediaItem{position:relative;display:flex;align-items:center;justify-content:center;box-sizing:border-box;min-width:0;max-width:100%;min-height:72px;max-height:180px;overflow:hidden;background:var(--bg,#f5f7fb);border:1px solid var(--line,#dce3ec);border-radius:8px;color:var(--muted,#6f7786);font-size:13px}.qbQuestionPreviewMediaItem img,.qbQuestionPreviewMedia>img{display:block!important;position:static!important;box-sizing:border-box!important;min-width:0!important;max-width:100%!important;max-height:180px!important;width:auto!important;height:auto!important;margin:0 auto!important;transform:none!important;box-shadow:none!important;border:0!important;border-radius:0!important;cursor:default!important;object-fit:contain!important}.qbQuestionPreviewMediaItem[data-pdf=true]::after{content:'PDF';position:absolute;right:5px;bottom:5px;padding:2px 5px;background:#000a;color:#fff;border-radius:4px;font-size:10px}.qbQuestionPreviewChoices{display:grid;gap:8px;min-width:0;max-width:100%;margin:14px 0 0;padding:0;list-style:none}.qbQuestionPreviewChoiceOption{display:grid;grid-template-columns:32px minmax(0,1fr);align-items:start;gap:8px;box-sizing:border-box;min-width:0;max-width:100%;padding:9px 10px;border:1px solid var(--line,#dce3ec);border-radius:9px;background:var(--card,#fff);font-size:14px;line-height:1.5;overflow-wrap:anywhere}.qbQuestionPreviewChoiceKey{display:flex;align-items:center;justify-content:center;min-width:30px;min-height:30px;border-radius:999px;background:var(--bg,#f5f7fb);color:var(--text,#172033);font-weight:900}.qbQuestionPreviewChoiceText{min-width:0;padding-top:4px;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word}
