...
function renderResult(Q,response,reviewOnly,refreshOnly=false){
  ...
  const off=`<div class="fbAnswerGroup"><b>解答</b>${fields.map((f,i)=>{const value=answerValue(a,f,i,fields);return`<div class="fbAnswerLine" data-answer-key="${esc(f.key)}" data-answer-text="${esc(value)}"><span>${esc(f.label)}</span><strong>${esc(value)}</strong></div>`}).join('')}${extraOfficial(a)}</div>`;
...
function bindPracticeBox(wrap,Q){
  wrap.querySelectorAll('.fbInput').forEach(x=>{resizeInput(x);x.addEventListener('input',()=>{resizeInput(x);syncButton(wrap)})});
  wrap.querySelector('#answer').onclick=async()=>{const response={};wrap.querySelectorAll('.fbInput').forEach(x=>response[x.dataset.k]=x.value.trim());const b=wrap.querySelector('#answer');b.disabled=true;try{await saveAnswered(Q,response);clearPracticeInputs(wrap);renderResult(Q,response,false)}catch(e){console.error(e);b.disabled=false;alert('解答履歴の保存に失敗しました')}};
...
window.QBFillBlankRefresh={
  ...
  async restore(Q,state){await enhancePractice();const box=document.querySelector('.fbBox');if(box){refreshPracticeFields(box,Q);box.querySelectorAll('.fbInput').forEach(x=>{x.value=state.values[x.dataset.k]||'';resizeInput(x)});syncButton(box)}if(state.revealed)renderResult(Q,state.values,state.review,true);}
};
...
function css(){if(document.getElementById('fbCssV2'))return;const s=document.createElement('style');s.id='fbCssV2';s.textContent=`.fbBox{margin-top:12px}.fbTitleRow{display:flex;justify-content:space-between;align-items:flex-start;gap:10px}.fbTitle{font-weight:900;font-size:15px;margin-bottom:4px}.fbAdminEdit{flex:0 0 auto;border:1px solid #cfe1ef;background:#fff;color:#126fb3;border-radius:8px;padding:6px 9px;font-size:11px;font-weight:900}.fbFields{display:grid;gap:8px;margin:12px 0}.fbField{display:grid;grid-template-columns:minmax(44px,auto) minmax(0,1fr);align-items:start;gap:8px;font-weight:900}.fbField>span{padding-top:10px}.fbInput{box-sizing:border-box;width:100%;max-width:100%;min-width:0;min-height:46px;border:1.5px solid #cfd8e3;border-radius:11px;padding:9px 11px;font:inherit;line-height:1.5;background:#fff;outline:none;resize:none;overflow-y:hidden;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word}.fbInput:focus{border-color:#126fb3;box-shadow:0 0 0 2px #126fb322}.fbAnswerGroup{margin-top:12px;padding-top:10px;border-top:1px solid #dce3ec}.fbAnswerLine{display:grid;grid-template-columns:minmax(44px,auto) minmax(0,1fr);gap:8px;margin-top:6px;line-height:1.5;min-width:0}.fbAnswerLine>strong{min-width:0;white-space:pre-wrap;overflow-wrap:anywhere}.fbExtra{margin-top:8px}.resultcard.fillblank{border:2px solid #126fb3;background:#f0f7fc}.fbAdminPanel{margin-top:12px;padding:12px;border:1px solid #cfe1ef;background:#f8fbff;border-radius:12px}.fbAdminHead{display:flex;justify-content:space-between;gap:8px;align-items:center}.fbAdminCols,.fbAdminRow{display:grid;grid-template-columns:80px 1fr 56px;gap:7px;align-items:center}.fbAdminCols{font-size:10px;color:#6f7786;margin-top:10px}.fbAdminRow{margin-top:7px}.fbAdminKey,.fbAdminLabel{width:100%;border:1px solid #cfd8e3;border-radius:8px;padding:8px;font:inherit;background:#fff}.fbAdminRemove{border:1px solid #e1b7b7;background:#fff;color:#b22;border-radius:8px;padding:8px 5px;font-size:11px;font-weight:900}.fbAdminActions{display:flex;gap:7px;align-items:center;flex-wrap:wrap;margin-top:10px}.fbAdminAdd,.fbAdminAuto,.fbAdminCancel,.fbAdminSave{border-radius:8px;padding:8px 10px;font-weight:900;font-size:11px}.fbAdminAdd,.fbAdminAuto,.fbAdminCancel{border:1px solid #dce3ec;background:#fff;color:#126fb3}.fbAdminSave{border:0;background:#126fb3;color:#fff}.fbAdminStatus{font-size:11px;color:#6f7786;flex:1;min-width:70px}@media(max-width:520px){.fbAdminCols,.fbAdminRow{grid-template-columns:64px minmax(0,1fr) 52px}.fbTitleRow{align-items:center}}`;document.head.appendChild(s)}
function boot(){css();schedule();['qb-screen-change','qb-app-ready','qb-content-updated'].forEach(ev=>window.addEventListener(ev,schedule));window.addEventListener('qb-retry-current',()=>{clearPracticeInputs(document.querySelector('.fbBox'));schedule()});document.addEventListener('click',dockCompat,true);const v=document.getElementById('view');if(v)new MutationObserver(schedule).observe(v,{childList:true,subtree:true})}
